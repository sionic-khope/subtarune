...
export class Player extends Character {
  ...
  /** 발소리: 발 딛는 프레임(1·3)으로 넘어가는 순간 중 지난 발소리에서 STEP_DIST(80px) 이상 걸었을 때만, 밟고 있는 타일이 `step`(이름 / 이름 배열 / {sfx, slices})을 선언했으면(얕은 물 — 옵젝영역) 그중 한 걸음을 살짝 음높이를 바꿔 재생하고 물결 고리를 낸다.
  ...
  footstep(prevFrame) {
    ...
    const s = tile.step, pick = { volume: 0.6, rate: 0.96 + Math.random() * 0.08 };
    if (s && s.slices) { const [from, len] = s.slices[Math.floor(Math.random() * s.slices.length)]; this.game.sound.sfx(s.sfx, { ...pick, from, len }); }   // 한 파일 안의 걸음 조각 하나
    else this.game.sound.sfx(Array.isArray(s) ? s[Math.floor(Math.random() * s.length)] : s, pick);
...
export class Raft extends Prop {
  ...
  jump(force = false) {
    ...
    for (const sw of this.swimmers) if (!sw.dead) { this.game.emitDropletsAt(sw.x + sw.w / 2, sw.y + sw.h, 10); sw.hop = 0; }   // 이륙 물튀김 + 웅크렸다 뛰는 연출
    return true;
  }
  ...
  update(dt) {
    ...
    if (this.jumping) {
      if (this.doubled) {                                                       // 2단: 포물선 (base + v t - g t^2 / 2), 0 이하 = 착지
        ...
        if (this.jumpY <= 0) { this.jumping = false; this.doubled = false; this.jumpT = 0; this.jumpT2 = 0; this.jumpY = 0; this.game.emitDropletsAt(this.x + this.w / 2, this.y + this.h * 0.7, 10); }
      } else {
        this.jumpT += dt;
    ...
    if (wall) {
      const need = wall.def.clear || 0, cx = nx + this.w / 2, inCore = cx >= wall.x - 8 && cx <= wall.x + wall.w + 8;

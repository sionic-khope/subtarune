...
//   이어서(같은 날 2부): ???: 떙땡씨~? → 최미스 느낌표, 가면을 위로 던져 씀 → 브금 gasuni 로 바뀌며 가순이1(긴머리)·2(단발)·3(땋은머리)이 위에서 내려옴 → 셋의 한 줄씩(3은 동동 뛰며) → 후훗
//   → 가순이들 오른쪽으로 → 최미스 왼쪽 봄·가순이들도 → 오른쪽 봄 → 내 추구미는 쵸소우야.(음성) → 가순이123 꺄아아악… → 카메라 나무 뒤(억빠맨·경섭) → 다시 → 이따 봐요 → 네 떙땡씨!!(달려 올라감)
//   → ㅋㅋㅋ... 후후..(브금 끔, 얼굴에 손, 가면을 땅에) → 휴우우… / 이렇게해서라도 … / 나는 상관없어!!!! / 으하하하 왼쪽길로 가야겠군(왼쪽 나무로 돌진, 나무가 날아감 — 사용자 2026-09-20, 순간이동 대신) → 눈 마주침 → 어. → 말풍선 넷 → ... 어 하이 → 억빠맨 ... / 스읍 미스
...
export const jjajang_glade_intro = [
  ...
  close,
  { zoom: 1, duration: 0.5 },
  // ── 2부: ???: 떙땡씨~? → 최미스 느낌표, 후다닥 디스코드 가면을 위로 던져서 쓴다
  { ...S(0, '떙땡씨~?'), speaker: '???' },
  close,
  ...
  // 나는 상관없어!!!! → 으하하하 왼쪽길로 가야겠군(사용자 2026-09-20 추가) → 왼쪽 나무(일행이 숨은 나무)로 돌진해 나무를 날려 버린다 → 일행이 드러나 눈이 마주침 → 어.
  C('나는 상관없어!!!!'),
  C('으하하하 왼쪽길로 가야겠군'),
  close,

...
// ─────────────────────────────────────────────────────────────
const H = (text, extra = {}) => ({ text, voice: 'narrator', ...extra });   // 나레이션(의성어·상태)
const HS = (text, extra = {}) => ({ speaker: '형섭', portrait: 'hyungsub', voice: 'hyungsub', text, ...extra });   // 형섭이 말하는 줄 (아직 인트로 맵 = 방)
...
export const pc_stream = Object.assign([
  { sfx: 'plug' },
  H('* 철컥..', { auto: 0.9 }),
  HS('* 큼큼..{w=0.5} 방송 세팅을 좀 하자.'),
  HS('* 헤이{w=0.3} 헤이유{w=0.3} 예아 유 ~'),
  HS('* ...{w=0.9}', { auto: 0.4 }),
  { chat: 'open' }, { chat: 'late' },                     // 채팅창 등장: 왜 늦었냐 + 엄/준/식
  { wait: 3.6 },
  HS('* 아 여러분 알았어요{w=0.4} 죄송합니다.'),
  HS('* 대신~{w=0.5} 제가 일요일 방송 키겠습니다'),
  { chat: 'spam' },                                       // 극 / 락 / ㅋㅋㅋㅋ 도배
  { wait: 2.8 },
  { chat: 'idle' },
  HS('* 아 여러분들 그리고 이제 오늘 떡밥으로 굴려볼 건 네..{w=0.7} 음?{w=0.6} 이게 뭐지?'),
  { sfx: 'error' },
  { dialog: { title: '오류', text: '보라색 코드에서 에러가 발생했습니다.', button: '해결하기' } },
  { chat: 'question' },                                   // ?? 류
  { wait: 0.8 },
  HS('* 뭐지 여러분들{w=0.4} 눌러볼게요~'),
  ...
  HS('* 어{w=0.5} 어{w=0.5} 어...'),
  { vortex: { size: 320, grow: 1.8 } }, { async: [{ shake: 1.6, amp: 5 }] },
  HS('* 어 어어 안된다{w=0.3} {shake}으아아아악!!{/shake}'),

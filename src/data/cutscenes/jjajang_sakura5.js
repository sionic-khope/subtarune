...
export const LEAP = { domijorim: { by: [-432, 66], height: 80 }, dohyun: { by: [-120, 66], height: 56 }, duration: 0.8 };   // 도미조림 착지 1550: 눕혔을 때 머리가 경섭에 안 닿게 30px 더 왼쪽(BUILD274)   // 둘이 뛰어 내려와 일행 양옆(같은 높이 발 380): 도미조림 x 1580(경섭 왼쪽), 도현 1812(억빠맨 오른쪽) → 일행을 본다
...
export const SAKURA5_BATTLE_AND_AFTER = [
  ...
  ...GIRLS.map(id => ({ face: id, dir: 'right' })),
  G5('그야 떙땡님께 환심을 살 수 있으니까요.'),
  G6('하지만 의미 없겠죠 이미..'),
  G4('맞아요 이미..'),
  // 억빠맨 물음표
  { emote: 'ppaman', kind: '?', duration: 1.0, hold: 0.7 },
  P('왜요?'),
  G4('곧 있으면 떙땡님이 청혼을 할거라고 하셨어요'),
  G5('그녀에게 청혼을 한다고 했어'),
  P('뭐 씨발 머야 그게'),
  G6('저도 믿기싫어요 떙땡님 ㅠ'),
  G6('그녀는 미자라구 하셨어'),

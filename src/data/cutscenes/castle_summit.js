...
export const SUMMIT_SCENE = Object.freeze({ stage: 'castle_summit_ready', cam: { talk: at(...VIEW_CENTER), face: at(...VIEW_CENTER), party: at(1720, 368), back: at(800, 368), far: at(820, 368), union: at(1560, 380) } });
...
export const castle_summit_confront = Object.assign([
  ...
  // 검은 연기가 한 번 팡 → 대사(BUILD382 사용자 “전환될 때 왜 눕는 거임, 눕는 거 빼”: 일행은 선 채로)
  ...PARTY.map(id => ({ face: id, dir: 'right' })),
  { wait: 0.6 }, summit(s => s.smokePop()), { wait: 1.2 },
  ...
  { bgm: null, fadeOut: 1.2 }, { camera: C.back, duration: 3.4 }, { wait: 0.3 },

...
// 주인공은 아래 문 기둥(x448~544) 안(456)에 서야 연출 뒤 그대로 내려가 나갈 수 있다
...
// 카메라: 일행과 그 앞 TV 를 화면 가운데에(x368 — 사용자 “이때는 가운데에 뜨는 게 맞지”) / 철창이 화면 오른쪽 위에서 내려옴 / 철창 + 왼쪽 TV(조롱) / 밧줄
const CAM_PARTY = { camera: [11.5, 6.6], duration: 0.6 }, CAM_CAGE = { camera: [15.5, 4.8], duration: 0.6 }, CAM_MOCK = { camera: [16.6, 4.2], duration: 0.6 }, CAM_ROPE = { camera: [19.4, 2.4], duration: 0.5 }, CAM_PARK = { camera: [20.4, 6.6], duration: 0.6 };
...
export const furnace_arena_intro = Object.assign([
  ...
  // 캐릭터들이 천천히 들어오고 느낌표 — 가운데 문에서 한 줄로 곧장 위로(axis:'y', x 는 문 자리 그대로). 사용자 “여기 기준이면 앞으로 갔다가 옆으로 갔다가 앞으로 가야지”(BUILD199c)
  //   한 줄 간격 44(맵 전환 직후 빠맨이 경섭 뒤 24px 에 겹쳐 있어 0.3초 늦게 출발시켜 벌린다). 맨 뒤 빠맨의 발판(y464~480)이 문 줄(row 15)에 안 걸리게 앞줄은 128
  ...
  bang(PARTY),
  // 후에 용암쪽 앞으로 온다 — 앞으로 → 옆으로 → 앞으로. 형섭이 앞장서 왼쪽으로 꺾고(모퉁이 y376), 경섭·빠맨은 뒤따라 같은 모퉁이까지 올라와 왼쪽으로(한 줄 44 간격이 그대로 뱀처럼 따라감),
  //   각자 제 자리 x(경섭 312 / 형섭 356 / 빠맨 400)에서 위로 꺾어 울타리 앞에 한 줄 — 빠맨이 먼저 꺾고 경섭이 형섭을 지나쳐 가장 왼쪽에서 꺾는다
  { parallel: PARTY.map((id, i) => [
    { move: id, rel: STAND, at: 'bottom', by: [0, 128], axis: 'y', speed: 60 },
    { move: id, rel: STAND, at: 'bottom', by: [[0, -44, 44][i], 128], axis: 'x', speed: 60 },
...
const AFTERMATH = [
  ...
  ...tvOn,
  { bgm: TV.bgm },
  // 사용자 2026-09-16 정정(“왼쪽 갔다 올라오면 됨”을 바꿈, 두 번째): “진지하게 눈 뜨면서 짜증나서 째려보는” glare 그림으로, 쉼표마다 한 줄. 마지막 줄만 어깨 으쓱(shrug)
  ...V('ㅇㅋ.. 인정해주겠음', 'glare'),
  ...V('근데 이제 다음은 없음', 'glare'),
  ...V('난 꼭.. 세상을', 'glare'),
  ...V('뭐 이런말 필요없나 곧 보자고.', 'shrug'),

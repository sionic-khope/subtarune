...
// 맵 좌표(tools/maps/youngcle18.py meta.tv / meta.cage): TV 프레임 최종 자리(일행 앞, 화면 위쪽)·철창 그림 왼쪽 위
const TV_X = 336, TV_Y = 100, CAGE_X = 432, CAGE_DROP = 332;
...
// TV: (x,y) 최종 자리의 360px 위(화면 밖)에 프레임·모니터암을 두고 드르르륵 내려온다 / 접혀서 올라간다
const tvPlace = (x, y) => ({ action: game => { const tv = ent(game, TVID), arm = ent(game, ARM); setPos(tv, x, y - 360); setPos(arm, x + 138, y - 360 - 320); } });
const tvDown = (dur = 2.0) => ({ parallel: [{ slide: TVID, by: [0, 360], duration: dur, sfx: 'rumble' }, { slide: ARM, by: [0, 360], duration: dur }] });
const tvUp = (dur = 1.1) => ({ parallel: [{ slide: TVID, by: [0, -360], duration: dur, sfx: 'rumble' }, { slide: ARM, by: [0, -360], duration: dur }] });
...
const CAM_PARTY = { camera: [15, 7.2], duration: 0.6 }, CAM_CAGE = { camera: [15, 4.5], duration: 0.6 };
const cageSway = { action: game => { const c = ent(game, CAGE); if (c) { c.def.oscillate = { dx: 7, period: 2.6 }; c.base = undefined; } } };
...
export const furnace_arena_intro = Object.assign([
  ...
  // 캐릭터들이 천천히 들어오고 느낌표
  { parallel: PARTY.map((id, i) => ({ move: id, rel: CAGE, at: 'bottom', by: [[0, -44, 44][i], 160], speed: 35 })) },
  ...PARTY.map(id => ({ face: id, dir: 'up' })),
  bang(PARTY),
  // 후에 용암쪽 앞으로 온다
  { parallel: PARTY.map((id, i) => ({ move: id, rel: CAGE, at: 'bottom', by: [[0, -44, 44][i], 64], speed: 60 })) },
  ...PARTY.map(id => ({ face: id, dir: 'up' })),
  P('와 ㅈㄴ 무섭네요'),
  G('그렇네..'),
  ...
  close,
  bang(PARTY),
  // 위에서 티비가 천천히 드르르륵 모니터암처럼 내려오고 — 캐릭터들은 뒷걸음(위를 본 채 미끄러져 물러남)
  CAM_PARTY,
  tvPlace(TV_X, TV_Y),
  { parallel: [tvDown(2.2), ...PARTY.map(id => ({ slide: id, by: [0, 36], duration: 1.3 }))] },
  ...tvOn,
  { bgm: TV.bgm },
  ...V('반갑노 게이들아', 'greet'),
  ...
  close,
  // 뒤로 카메라 이동하고 새장 철창에 갇힌 쥰희·용준이 밧줄에 매달려 덜렁 내려온다 → 좌우로 주렁주렁
  CAM_CAGE,
  ...
  ...tvOff, tvUp(),
  tvPlace(CAGE_X - 300, 40),
  { camera: [11.5, 4.5], duration: 0.6 },
  tvDown(1.6),
  ...tvOn,
  ...
  // 티비가 접히고 다시 주인공쪽으로 카메라, 티비가 주인공 앞으로 다시 내려온다
  ...tvOff, tvUp(),
  CAM_PARTY,
  tvPlace(TV_X, TV_Y),
  tvDown(1.6),
  ...
  close,
  { camera: [15, 2.6], duration: 0.5 }, { wait: 0.9 }, CAM_PARTY,
  ...
  { action: game => {
    ...
    const c = ent(game, CAGE); if (c) { c.def.oscillate = { dx: 7, period: 2.6 }; c.base = undefined; }
    game.finishTvBroadcast(); game.tvBroadcast = new TvBroadcast(game, TV);
    const tv = ent(game, TVID), arm = ent(game, ARM);
    if (tv) setPos(tv, TV_X + 160, TV_Y - 20); if (arm) setPos(arm, TV_X + 160 + 138, TV_Y - 20 - 320);

...
export const VOICES = {
  choimis_flower: { freq: 245, wave: 'triangle', dur: 0.14, jitter: 12, gain: 0.18, cutoff: 2400, rate: 1, level: 0.8, cut: true, minGap: 0.09 },
  ...
  // 최미스(BUILD258 사용자 “보이스폰트는 그냥 그 델타룬의 킹 목소리로”): voices/choimis.mp3 = 델타룬 킹(랜서 아빠) 대사음 snd_dadtxt 그대로(assets/source/choimis-voice-v1). 합성값은 파일 없을 때 폴백
  choimis: { freq: 160, wave: 'square', dur: 0.12, jitter: 24, gain: 0.2, cutoff: 1400, rate: 1, level: 0.9, cut: true, minGap: 0.08 },
  gasuni: { freq: 470, wave: 'triangle', dur: 0.12, jitter: 30, gain: 0.17, cutoff: 2600, rate: 1, level: 0.85, cut: true, minGap: 0.1 },   // 가순이 셋(BUILD258 사용자 “언더테일 토리엘 목소리에서 톤을 높인 버전”): voices/gasuni.mp3 = 델타룬 snd_txttor 를 1.32배 높인 것(assets/source/gasuni-voice-v1). 합성값은 폴백   // level 0.85 → 0.72 (-15%, 사용자 “청소부 목소리 15퍼만 크기 줄여”)
...
export class Sound {
   ...
   */
  playBgm(name, { loop = true, volume = 0.35, fadeIn = 0.5, loopEnd = 0, loopFade = 0.8, then = null, at = 0 } = {}) {
    ...
    a.loop = then ? false : loop; a.volume = 0;
    // then: 이 곡이 끝나면 바로 이어 반복할 곡(BUILD369 SAVE The World — 처음 한 번은 도입부부터, 반복은 하이라이트부터 공백 없이)
    if (then) {
      this.preloadBgm(then);
      a.addEventListener('ended', () => { if (a !== this.bgm) return; this.bgm = null; this.bgmName = null; this.playBgm(then, { volume: this.bgmVolume, fadeIn: 0 }); }, { once: true });
    }

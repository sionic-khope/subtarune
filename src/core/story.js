...
export const STAGES = [
  ...
  { id: 'castle_arena_seen', desc: '결전지 · 가재맨과 섭타룬, 거대한 푸른 파동', map: 'gajaeman_castle_arena', spawn: 'rim' },
  { id: 'castle_summit_ready', desc: '꼭대기 · 청소년과 가재맨의 대치(전투 직전)', map: 'gajaeman_castle_summit', spawn: 'confront' },
];
...
QA_POINTS.push({ ...invasionReady, id: 'gajaeman_castle_entry', desc: '가재맨 성 입구 · 도착 연출 완료 후 세 사람 조작',
  ...
  flags: { ...invasionReady.flags, ship_invasion_started: true, ship_rally_done: true, ship_deck_bond_done: true, ship_invasion_arrived: true,
    shop_yongjun_strong_cialis: true, shop_yongjun_strong_vaseline: true },
...
const gateReady = QA_POINTS.find(point => point.id === 'castle_left_orb_after');
QA_POINTS.push({ ...gateReady, id: 'castle_gate_reunion', desc: '두 봉인 완료 · 로비 대문 개방과 동료 진입',
  map: 'gajaeman_castle_lobby', spawn: 'gate_reunion', flags: { ...gateReady.flags } });
for (const [id, map, spawn, desc] of [
...
for (const [id, spawn, desc] of [
  ...
  party: ['gyeongsub', 'ppaman'], flags: { ...gateReady.flags, castle_gate_open: true, castle_gate_reunion_done: true, castle_cathedral_rescue_done: true } });
for (const [id, spawn, extra, desc] of [
  ['castle_stairs', 'start', {}, '무너지는 계단 · 입구(주먹이 뒤 계단을 부수고, 몬스터·누누와 윌럼프)'],
  ['castle_stairs_top', 'top', { castle_stairs_monsters_done: true, castle_stairs_nunu_done: true }, '무너지는 계단 · 꼭대기'],
]) QA_POINTS.push({ ...gateReady, id, desc, map: 'gajaeman_castle_stairs', spawn, stage: 'castle_arena_seen',
  party: ['gyeongsub', 'ppaman'], flags: { ...gateReady.flags, castle_gate_open: true, castle_gate_reunion_done: true, castle_cathedral_rescue_done: true, castle_prophecy_door_done: true, ...extra } });
for (const [id, spawn, extra, desc] of [
...
// BUILD353 청소년전 QA 지점(사용자 “qa지점 추가도”): 전투 직행 · 2페이즈 직행 · 격파 뒤 연출 · 연출 뒤
{
  ...
  for (const [id, map, spawn, extra, desc] of [
    ...
    ['ship_lounge_farewell', 'ship_lounge_farewell', 'start', { ...road, castle_road_done: true, castle_raft_launched: true, castle_sunset_arrived: true, castle_gajaeman_clash: true, castle_epilogue_done: true, ship_lounge_epilogue_seen: true, ship_deck_epilogue_seen: true }, '라운지 마지막 작별 → 엔딩 크레딧'],
    ['ship_credits', 'ship_lounge_farewell', 'start', { ...road, castle_road_done: true, castle_raft_launched: true, castle_sunset_arrived: true, castle_gajaeman_clash: true, castle_epilogue_done: true, ship_lounge_epilogue_seen: true, ship_deck_epilogue_seen: true, ship_lounge_farewell_seen: true }, '엔딩 크레딧 바로'],

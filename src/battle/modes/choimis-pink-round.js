...
const HEART_X = 110;
const PINK = '#ff5ca8';
const HINT_KEYS = Object.freeze({ choso: 'battle_choimis_pink_choso', kart_block: 'battle_choimis_pink_kart', pink_prism: 'battle_choimis_pink_prism' });
...
/** Three unique projectile contacts become one common one-HP battle hit. */
...
/** Enemy-mode adapter for the three post-tutorial Choimis shooting rounds. */
export function createChoimisPinkRound(battle, { enemy, config }) {
  ...
  let phase = 'prep', prepHold = 0, elapsed = 0, shots = [], effects = [], nextShotId = 0, disposed = false, terminating = false;
  ...
  const scenario = createChoimisPinkScenario(scenarioName, {
    box: BOARD, soul, images, hit, hurt, bossContact,
  ...
  const terminate = () => {
    ...
    terminating = true; shots = []; effects = []; fireControl.dispose(); shotAudio.stop(); scenario.dispose?.(); battle.bubble = null;
  };
  return {
    get snapshot() {
      return { phase, elapsed: Math.round(elapsed * 100) / 100, scenario: scenario.snapshot,
        ...
        bossHits: enemy.pinkShotHits || 0, terminating, disposed };
    },
    update(dt, input) {
      if (disposed) return true;
      if (enemy.hp <= 0 || enemy.dying > 0 || enemy.dead) { terminate(); return !!enemy.dead; }
      if (scenario.done) return true;
    ...
    draw(ctx) {
      ...
      ctx.font = FONT.replace(/^\d+px/, '12px'); ctx.fillStyle = '#ffb4d7'; ctx.fillText(L.battle_choimis_pink_round_controls, 34, 280);
    },
    dispose() {
      if (disposed) return;

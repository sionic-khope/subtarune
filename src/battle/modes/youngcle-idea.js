...
function ideaBall(battle, member, yc) {
  ...
  const finishHit = () => { battle.sfx(C.sfx.kieek); battle.sfx('boom', { volume: 0.7 }); yc.patternPose = { sheet: 'surprise', frame: 1 }; battle.game.shake = { time: 0.4, amp: 6 }; battle.hitEnemy(yc, member, C.ideaDamage, { source: 'idea', sound: false }); shout = '펑!'; };
  return {
    ...
    update(dt, input) {
      ...
      if (phase === 'enter') { const r = rect(); const k = Math.min(1, pt / 0.6); P.x = member.home[0] + (r.x + r.w * 0.3 - member.home[0]) * k; P.y = member.home[1] + (r.y + r.h / 2 - member.home[1]) * k; if (k >= 1) { setPhase('morph'); battle.sfx('whoosh', { volume: 0.5 }); } return false; }
      ...
      if (phase === 'talk2') { if (talk.update(dt, input)) { setPhase('game'); battle.setText(''); const r = rect(); N.x = r.x + r.w + 30; N.y = r.y + r.h * 0.4; N.vx = -K.naram.speed; N.vy = K.naram.speed * 0.45; N.on = true; if (naram) naram.patternPose = { hidden: true }; battle.sfx('jump', { volume: 0.5 }); } return false; }
      if (phase === 'game') {
        ...
        if (input.just('confirm') && sp > 8 && dashT <= 0) { P.vx = P.vx / sp * K.ball.dash; P.vy = P.vy / sp * K.ball.dash; dashT = K.ball.dashTime; battle.sfx('whoosh', { volume: 0.6 }); }
        if (dashT > 0) dashT -= dt;
    ...
    draw(ctx) {
      const r = rect();
      if (phase === 'talk' || phase === 'talk2') { if (phase === 'talk2') { board.draw(ctx); drawBall(ctx, ballImg, 1, P.x, P.y, P.r * 2 + 10); } battle.drawTextBox(ctx); return; }
      if (['enter', 'morph', 'game', 'launch', 'impact', 'revert'].includes(phase)) {
        board.draw(ctx);
        ctx.save(); ctx.beginPath(); ctx.rect(r.x + 3, r.y + 3, r.w - 6, r.h - 6); ctx.clip();
        if (N.on && phase === 'game') drawNaramBall(ctx, naramImg, N.x, N.y, N.r, N.rot);
...
function ideaQuiz(battle, member, yc) {
  ...
  let phase = 'talk', talk = createTalk(battle, K.before), pt = 0, q = 0, disposed = false, zoom = 1, flames = [], berserkTalk = null, obPose = null, hitDone = false;
  const setPhase = (p) => { phase = p; pt = 0; };
  ...
  return {
    fullscreen: true,
    get snapshot() { return { phase, q, zones: phase === 'quiz' ? zones() : null, soul: { x: Math.round(soul.x), y: Math.round(soul.y) }, obangsunGone: !ob || ob.dead, ycHp: yc.hp }; },
    update(dt, input) {
      if (disposed) return true; pt += dt;
      if (phase === 'talk') { if (talk.update(dt, input)) { setPhase('quiz'); board.setTarget(240, 160, 240, 214); board.snap(); soul.center(board.rect); battle.setText(''); } return false; }
      if (phase === 'quiz') {
        soul.update(dt, input, board);
        if (input.just('confirm')) { const zs = zones(); const pick = zs.findIndex(inZone);
          if (pick >= 0) { if (pick === K.quiz[q].answer) { battle.sfx('confirm'); q++; if (q >= K.quiz.length) { setPhase('insult'); board.setTarget(440, 72, 240, 282); talk = createTalk(battle, K.insult); } }
      ...
      if (phase === 'insult') { if (talk.update(dt, input)) { battle.game.sound.stopBgm(0.4); setPhase('dots'); talk = createTalk(battle, K.dots); } return false; }
      if (phase === 'dots') { if (talk.update(dt, input)) { setPhase('closeup'); battle.setText(''); } return false; }
      ...
      if (phase === 'berserk') {
        ...
        if (berserkTalk.update(dt, input) && pt > 2.4) { setPhase('leap'); battle.sfx('whoosh', { volume: 0.8 }); if (ob) ob.leapFrom = { ...obPose }; }
        return false;
      }
      if (phase === 'leap') { if (ob) { const k = Math.min(1, pt / 0.45); ob.patternPose = { x: ob.leapFrom.x + (yc.x + 10 - ob.leapFrom.x) * k, y: ob.leapFrom.y + (yc.y - 10 - ob.leapFrom.y) * k - 70 * k * (1 - k) }; if (k >= 1 && !hitDone) { hitDone = true; battle.sfx(C.sfx.kieek); battle.sfx('boom', { volume: 0.7 }); yc.patternPose = { sheet: 'surprise', frame: 1 }; battle.game.shake = { time: 0.4, amp: 6 }; battle.hitEnemy(yc, member, C.ideaDamage, { source: 'idea', sound: false }); setPhase('flee'); } } else setPhase('flee'); return false; }
    ...
    draw(ctx) {
      ...
      if (phase === 'quiz') {
        board.draw(ctx); const r = board.rect, item = K.quiz[q], zs = zones();
        ctx.fillStyle = '#fff'; ctx.textAlign = 'center'; ctx.fillText(item.q, 240, r.y + 10);
        zs.forEach((z, i) => { const on = inZone(z); ctx.fillStyle = on ? '#3a3000' : '#111'; ctx.fillRect(z.x, z.y, z.w, z.h); ctx.strokeStyle = on ? '#ffe066' : '#9a9ab0'; ctx.lineWidth = 2; ctx.strokeRect(z.x + 1, z.y + 1, z.w - 2, z.h - 2); ctx.fillStyle = on ? '#ffe066' : '#fff'; ctx.font = FONT.replace(/^\d+px/, '13px'); if (ctx.measureText(item.a[i]).width > z.w - 10) ctx.font = FONT.replace(/^\d+px/, '11px'); ctx.fillText(item.a[i], z.x + z.w / 2, z.y + 13); ctx.font = FONT; });
        ctx.fillStyle = '#c9c9d9'; ctx.font = FONT.replace(/^\d+px/, '12px'); ctx.fillText('칸에 들어가서 C', 240, r.y + r.h + 6); ctx.font = FONT; ctx.textAlign = 'left';
        soul.draw(ctx);
      } else if (['talk', 'fail', 'insult', 'dots', 'berserk', 'after'].includes(phase)) battle.drawTextBox(ctx);

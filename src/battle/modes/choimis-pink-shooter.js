...
export function createChoimisPinkShooter(battle, { enemy }) {
  ...
  let phase = 'open', phaseTime = 0, combatElapsed = 0, tailElapsed = 0, water = 0, scroll = 0;
  ...
  const updateCombat = (dt, input) => {
    if (phase === 'combat') combatElapsed = Math.min(C.combatSeconds, combatElapsed + dt);
    else tailElapsed += dt;
    phaseTime += dt; scroll += dt * 92;
    ...
    while (phase === 'combat' && combatElapsed >= nextSpawn && nextSpawn < C.combatSeconds - 0.8) { spawnNoodle(); nextSpawn += C.spawnEvery; }
    ...
    if (phase === 'combat' && combatElapsed + 1e-9 >= C.combatSeconds) change('tail');
    if (phase === 'tail' && !noodles.length) { change('settle'); fireControl.dispose(); shotAudio.stop(); }
    if (phase === 'settle' && !shots.length && !effects.length) change('done');
  };

  return {
    get snapshot() {
      return {
        phase, phaseTime: Math.round(phaseTime * 100) / 100, combatElapsed: Math.round(combatElapsed * 100) / 100, tailElapsed: Math.round(tailElapsed * 100) / 100,
        ...
        charge: { ...fireControl.snapshot, aura: fireControl.snapshot.active ? pinkChargeAura(soul.x, soul.y, fireControl.snapshot.progress, combatElapsed + tailElapsed) : [] },
      };
    },
    update(dt, input) {
      ...
      } else updateCombat(phase === 'combat' ? Math.min(delta, C.combatSeconds - combatElapsed) : delta, input);
      return phase === 'done';
    },
    draw(ctx) {
      ...
      if (['launch', 'combat', 'tail', 'settle'].includes(phase)) drawPinkScroll(ctx, b, scroll);
      if (water > 0) drawPinkWater(ctx, b, water, phaseTime);
      for (const noodle of noodles) drawNoodle(ctx, noodle, bowl.image);
      if (fireControl.snapshot.active) drawPinkChargeAura(ctx, pinkChargeAura(soul.x, soul.y, fireControl.snapshot.progress, combatElapsed + tailElapsed), fireControl.snapshot.ready);
      ...
      ctx.globalAlpha = 1; drawHeartShape(ctx, soul.x, soul.y, forming ? '#ff203a' : PINK, forming ? 'down' : 'right', soul.invuln, combatElapsed + tailElapsed);

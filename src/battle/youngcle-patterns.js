...
export const YOUNGCLE_PATTERNS = {
  ...
  obangsun_rays: (o = {}) => {
    const dur = o.duration ?? 7.8, every = o.every ?? 1.45, warn = o.warn ?? 0.45, first = o.first ?? 0.9, faceR = o.faceR ?? 44, open = o.open ?? 0.9, rays = o.rays ?? 4, flames = o.flames ?? 6, flameSpeed = o.flameSpeed ?? 85;
  ...
  naram_slam: (o = {}) => {
    const walk = o.walk ?? 2.2, rise = 0.5, hang = 0.7, fall = 0.26, debris = o.debris ?? 18, dur = o.duration ?? 8.6, scale = o.scale ?? 1.5, waveSpeed = o.waveSpeed ?? 170;
    const tJump = walk, tTop = tJump + rise, tHang = tTop + hang, tLand = tHang + fall;
    let body = null, mark = null, landed = false, jumped = false, debrisLeft = debris, nextDebris = 0, landX = 0, startX = 0;
    return { duration: dur, update(t, dt, api) {
      ...
      if (t < tJump) { body.x = startX + (landX - startX) * (t / walk); body.y = floor; body.frame = Math.floor(t * 5) % 4; body.rot = 0; body.harmless = true; }
        ...
        if (!landed) { landed = true; body.rot = 0; body.y = floor; body.frame = 0; api.sfx?.('baron_slam'); api.shake?.(0.5, 8); nextDebris = t + 0.15;
          for (const dir of [-1, 1]) api.emit({ x: landX + dir * 30, y: floor - 9, vx: dir * waveSpeed, vy: 0, r: 9, kind: 'white', shape: 'wave', life: 3, box: { ...b },
            drawShape: (ctx, w) => { ctx.save(); clipBox(ctx, w.box); const x = Math.round(w.x), y = Math.round(w.y); ctx.fillStyle = '#fff'; ctx.fillRect(x - 7, y - 9, 14, 18); ctx.fillStyle = '#000'; ctx.fillRect(x - 4, y - 5, 8, 10); ctx.fillStyle = '#fff'; ctx.fillRect(x - 2, y - 2, 4, 4); ctx.restore(); } }); }
        body.rot = 0; body.harmless = t > tLand + 0.35;
        if (debrisLeft > 0 && t >= nextDebris) { debrisLeft--; nextDebris = t + 0.13; const x = b.x + 10 + api.rnd() * (b.w - 20);
          api.emit({ x, y: b.y + 4, vy: 40 + api.rnd() * 60, ay: 280, r: 4 + api.rnd() * 4, shape: 'rock', kind: 'white', spin: (api.rnd() - 0.5) * 6 }); }
  ...
  youngcle_orbit_laser: (o = {}) => {
    const dur = o.duration ?? 10.4, speed = o.speed ?? 215, rev = o.rev ?? 0.42, aimT = o.aim ?? 0.22;
    const shots = o.shots ?? [0.8, 1.15, 1.5, 3.4, 3.75, 5.3, 5.65, 6.0, 7.6, 7.95, 8.3, 8.65], laughs = o.laughs ?? [2.3, 4.7, 7.0];
    ...
    return { duration: dur, update(t, dt, api) {
      ...
      while (si < shots.length && t >= shots[si]) {
        ...
        const angles = idx % 3 === 2 ? [-0.32, 0, 0.32] : [0];

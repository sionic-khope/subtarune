...
// 양옆(경섭 드럼·빠맨 보컬)도 두 칸씩이고 자동으로 친다(사용자 확정). 노트는 탭·홀드 두 종류, 판정은 GREAT/MISS 둘, 콤보가 쌓인다. MISS 마다 파티 HP 가 10 깎이고(씬이 처리) 누가 0 이 되면 게임오버(재시도). 인기(POPU/LARITY)는 GREAT 로 차고 MISS 로 준다.
// 홀드(BUILD182 사용자 확정): 머리를 맞히고 꾹 누르다 떼면 성공 — 중간에 떼도 MISS 가 아니다. 끝까지 누르면 보너스 전부, 일찍 떼면 누른 비율만큼(최소 절반).
export const LANES = ['L', 'R'];
export const RHYTHM = {
  approach: 1.15,       // 노트가 위에서 판정선까지 내려오는 시간(초) — BUILD190 사용자 “속도 높이고 간격 넓히되 적당히”(1.5→1.15, 차트 타이밍은 그대로)
  great: 0.16,          // ±이 안에 누르면 GREAT — BUILD190 “판정 좀 더 후하게”(0.12→0.16)
  late: 0.2,            // 이만큼 지나면 MISS(0.16→0.2)
  holdRelease: 0.15,    // (참고) 홀드 끝 여유 — 이제 떼는 시점은 판정에 안 쓰이고 보너스 비율에만 반영
  missLimit: Infinity,  // 연속 MISS 게임오버 — BUILD190 사용자 “5번 틀리면 실패가 아니라 틀릴 때마다 체력 10씩”: 이제 씬이 파티 HP 로 끝을 정한다(연속 미스 한도 없음)

 ...
 * 곡 `save_the_world_full` = 사용자 지정 LAn-JYzKm5M 의 42.7초(음이 바뀌는 지점)부터 1:47까지 한 곡(BUILD376 — 상승·착지·SAVE THE WORLD·결전이 끊김 없이 한 곡).
 * 시간 T 는 그 곡의 재생 위치(없으면 자체 시계). 곡 박자 약 180BPM, 한 마디 1.33초(원곡 63.95·65.28·66.61초 마디 첫 박).
 ...
 *   T 18.26  (원곡 60.96초, BUILD378) 앞덤블링으로 올라와 무릎 꿇고 착지 — 챱 → 63.95초 하이라이트에 달리기(RISE.press/white/dash)
 * 선 모양 효과는 쓰지 않는다(사용자 “선적인건 싫어”): 빛은 부드러운 덩어리·흐린 광선, 입자는 도트 덩어리.
 */
export const RISE = Object.freeze({
  bgm: 'save_the_world_full',
  // BUILD376(사용자 “눌러서가 아니라 브금 타이밍 맞춰 착지하자마자 바로, 브금 하나로”): 착지(64.05) 직후 박(64.3)에 SAVE THE WORLD 가 저절로 눌리고
  //   흰 빛이 다음 마디(65.28)까지 화면을 감싼 뒤 그림자 준비 동작 → 그다음 마디(66.61)에 출발. 원곡 초 − 42.7 = T
  //   BUILD378(사용자 “하늘 상승은 43초부터, 달리기 때 검 뽑는 소리가 나고 이어서 SAVE THE WORLD 하이라이트(1분 4초)가 같이”):
  //   곡 분석 — 42.7초부터 조용한 빌드업, 63.95초에 큰 한 방(하이라이트), 박 0.333초(180BPM).
  //   원곡 55.46 바다·해 열림(악센트) · 57.46 흰 번쩍임→노을 땅(악센트) · 60.96 착지 · 61.29 저절로 눌림 · 62.62 흰 화면 · 63.80 출발(검 뽑는 소리) → 63.95 하이라이트

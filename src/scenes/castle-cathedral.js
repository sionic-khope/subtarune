...
export class CastleCathedral {
  constructor(game, { rnd = Math.random } = {}) {
    ...
    const meta = game.map?.def?.meta?.cathedralClimb;
    this.cfg = { ...CATHEDRAL, ...(meta && typeof meta === 'object' ? meta : {}) };
  ...
  progress() {
    ...
    return clamp01((this.cfg.startY - p.y) / (this.cfg.startY - this.cfg.topY));
  }
  ...
  placeAtTop() {
    ...
    [this.actor.x, this.actor.y] = this.cfg.top; this.actor.visible = true;
  ...
  panToPlayer(seconds) {
    ...
    this.climbing = true; this.updateFocus(); this.climbing = climbing;
  ...
  update(dt) {
    ...
    if (a && this.beat === 'leave') {
      const k = clamp01(this.elapsed / c.duration.leave);
      a.y = this.riseFrom - c.leaveHeight * k * k;
      if (k === 1) { a.visible = false; this.fan = 0; }
    }
    if (this.beat === 'forge') {
    ...
    for (const s of this.swords) {
      ...
      if (s.phase === 'charge') {
        ...
        if (s.t >= (s.charge ?? c.charge)) { s.phase = 'fall'; s.t = 0; cut = true; }
      ...
      } else if (s.phase === 'broken' && s.t > 0.4) s.phase = 'done';
    }
    if (cut) this.sound('knight_cut', 0.7);
    this.swords = this.swords.filter(s => s.phase !== 'done');
    // the boss moves on to the next hall once the climber nears the top (BUILD325)
    if (c.leaveY && p.y < c.leaveY && this.beat === 'idle' && a?.visible) this.setBeat('leave');
    const armed = p.y < c.startY && p.y > c.stopY && this.volleysAllowed();
    this.nextVolley -= seconds;
    if (armed && this.nextVolley <= 0 && !this.swords.some(s => s.phase === 'charge')) {
      this.spawnVolley(progress, laneOf(p.x + p.w / 2));
    }
    this.afterSwords(seconds);
  }
  /** Map-specific gate for new volleys (part 2 pauses them during its rescue scene). */
  volleysAllowed() { return true; }
  /** Hook for part-2 laser support; the base climb has nothing to add. */
  afterSwords() {}
  spawnVolley(progress, playerLane) {
    const c = this.cfg, cam = this.game.camera;
    for (const lane of pickVolley(progress, playerLane, this.rnd)) this.swords.push({ lane, phase: 'charge', t: 0, y: cam.y + c.chargeTop, charge: c.charge });
  ...
  drawFan(ctx, cam, image) {
    ...
    const tx = this.actor.x, ty = this.actor.y, head = ty - 96, bob = Math.sin(this.time * 1.6) * 3;
  ...
  draw(ctx, cam) {
    if (this.disposed) return;
    const c = this.cfg, image = this.game.propImages[c.sword];
    if (image) this.drawFan(ctx, cam, image);
    for (const s of this.swords) {
      if (s.phase === 'broken') continue;
      const x = c.lanes[s.lane] - cam.x, rect = swordHitRect(s);
      if (s.phase === 'charge') {
        const k = clamp01(s.t / (s.charge ?? c.charge)), pulse = 0.18 + 0.22 * k + (Math.floor(s.t * 14) % 2 ? 0.08 : 0);
  ...
  drawPlayer(ctx, cam, player, drawBody) {
    ctx.save(); ctx.globalAlpha *= this.cfg.heartAlpha; drawBody(); ctx.restore();
    const g = this.game;

...
**실행기 (2026-09-14 저녁 추가):** `tools/sprites/imagegen.py`는 stdlib만 쓰는 OpenAI 호환 클라이언트다(절차 [subtarune-imagegen](../../subtarune-imagegen/SKILL.md)). `models --images`로 카탈로그를 조회하고, `generate`는 참조 없음→`POST /v1/images/generations`(JSON), `--ref`→`POST /v1/images/edits`(위 실측대로 **단일 `image` 파일 필드**, `input_fidelity` 미전송)를 보낸다. 참조 여러 장은 `compose`로 한 PNG에 가로 결합해 보내며, 약1MB를 넘는 참조에는 413 경고를 낸다. 기본 모델 `openai/gpt-image-2`, base `https://apis.opengateway.ai/v1`, 키는 `OPENGATEWAY_API_KEY` 환경변수 → repo `.env` → `~/.hermes/.env` → macOS 키체인 `og-api-key`(hermes `providers.og.key_cmd`와 동일) 순으로 도구 내부에서만 읽는다. 응답의 `b64_json`/`url` 둘 다 처리하고 `<out>.prompt.txt`/`<out>.meta.json`(usage, 키 없음)을 남긴다. 게임 저장소 실측(같은 날 저녁, 키체인 경로): `models --images` 10개, `openai/gpt-image-2` 1024x1024 high 소품 1장 `images/generations` 성공(97.6s, usage input 179 / output 7,024 image tokens / total 7,203), 저장소 processor `tools/sprites/sheet_processor.py` strict-QC 통과, NEAREST export·이진 알파·마젠타 잔여 0 — 증거 `assets/source/imagegen-test/README.md`. 이 실행기로 `images/edits`를 실제 호출한 기록은 아직 없으며(스튜디오 `tools/og-image/og-image.mjs` 실측만 있음) 첫 참조 편집 작업에서 남긴다. 실결제액은 대시보드 로그로 확인한다.

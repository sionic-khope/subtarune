...
- **상자(공격 칸) 안에 들어오는 캐릭터 그림은 흰색 간소화 도트**(델타룬 탄막 규칙, 사용자가 여러 번 지적 — 2026-09-17 조종실 전투에서 또 놓침). 원색 스프라이트를 그대로 상자에 넣지 않는다. `src/battle/youngcle-patterns.js` 의 `whiteSprite(img)`(gfx.monoPortrait 흰/검 2톤)로 변환해 그린다. 얼굴·몸통·탈것 전부.
- **모든 패턴에 준비 시간(예고)**: 탄이 오기 전에 조준선·착지 표식·내려오는 철창·깜빡이는 띠처럼 “어디로 올지”가 먼저 보이고(≥0.3s), 조작이 필요한 기믹은 시작 전에 조작 설명 대사/나레이션이 나온다(억빠맨 볼: “저를 방향키로 조정해주세요…” + 나레이션). ‘뿅’ 하고 나타나는 상자·문제·탄 금지 — 상자는 tween 으로 열리고 내용은 타자/떠오름으로 나타난다.
- **회피 기믹의 위험 영역은 고정 예고 → 고정 발사**(2026-09-17 포스트모텀 #21 `docs/postmortems/2026-09-17-ship-battle-third-pass.md`): 발사 중 영역이 하트를 따라가는 설계(빔 스윕)는 “움직이면 절대 못 피한다”. 난도는 예고 시간·연타 횟수·탄 수로. 피해는 **평타 15 고정**, 피격 누적 계단·배수 금지(`damageStep 0`). 잡담은 사용자 말투로 최소 4줄.
- **난이도 기본값은 “델타룬 보스전”**: 탄 몇 개가 천천히 지나가는 패턴은 사용자에게 “너무 쉬워”로 돌아온다(2026-09-17). 한 패턴 안에 회피 축이 둘 이상(예: 광선 + 불덩이, 착지 + 충격파 + 잔해, 조준 볼트 + 3갈래), 지속 위험(3초 빔) 하나, 연타 기믹은 초당 5회 이상을 요구한다. 피격마다 적 공격력이 오르는 규칙(`support.partyDamage`)이 있으면 회피 실패의 대가가 커진다 — 수치는 데이터 파일에.
- **소리는 델타룬(TeamBlossomDevs/DeltaruneDecomp_beta `sounds/`)에서 먼저 찾는다**. 이미 `assets/audio/sfx` 에 있는 델타룬 소리(jump, impact, punch, hit, locker, spearappear, heavyswing, wing, baron_slam, furnace_blast, queen_hoot …)를 재사용하고, 없을 때만 받아 온다. ffmpeg 합성은 사용자가 “없어 보인다”고 했다(laser_pew/laser_fire 폐기). `whoosh`(합성 바람)는 사용자가 “파도소리”라며 싫어한다 — 어디에도 기본값으로 쓰지 않는다. 피함 = 검 소리(`hit`).
- **캐릭터가 그려지는 새 그림은 gpt-image-2.5-sunburst 로 생성**(필드·전투·놀람·쓰러짐·탈것·큰 소품). 손으로 PIL 합성하지 않는다. 있는 그림은 재사용(같은 캐릭터 새 포즈는 기존 시트를 참조로). 전투 스프라이트는 옆모습 정면이 아니라 **왼쪽 아래를 보는 3/4 시점**, 표정은 캐릭터 성격(영클 = 비웃음).
- **텍스트는 상자 안에 들어와야 한다**: 문제·선택지·안내 문구는 `measureText` 로 재서 줄바꿈하거나 글꼴을 줄인다. 상자 폭은 가장 긴 문장 기준으로 정한다(퀴즈 320px). 스크린샷에서 글자가 테두리에 걸치면 실패.
- **정답·대사·순서는 브리핑 원문과 대조**해서 표로 남긴다(`design/narrative/cutscenes/<전투>.md` 구현 대응표). 사용자가 나중에 정정한 값(퀴즈 1번 = 컴퓨터 사기)은 원문 아래에 정정 줄로 적고 코드 주석에도 날짜를 남긴다.
- **‘...’ 은 말풍선**(머리 위 흰 풍선)이지 대사 상자 줄이 아니다. 브금을 끌 때는 페이드아웃(≥1s).
- **연출 방향은 장면의 논리로**: 뒤통수를 치려면 적이 보는 반대쪽(영클은 왼쪽을 보니 오른쪽)에서 들어온다. 흰 화면으로 끝나는 연출은 전투 기본 화면이 다시 보이면 안 된다(`battle.finish(true, { white: true })`).

...
새 맵·퍼즐·기믹 배치는 [맵 구성 판단](../../../docs/development/map-design.md)을 먼저 적용한다. 공연장처럼 배우·관중·조명이 이어지는 공간은 [연출 판단](../../../docs/development/staging.md)도 읽는다. 아래 역사적 사례의 폭·색·빛 모양은 해당 사례의 값이며 새 브리핑보다 우선하지 않는다.
...
먼저 `docs/STATE.md`의 해당 최신 항목을 읽는다. 맵은 데이터(`assets/maps/<id>.json`)다. 생성기가 있는 맵은 생성기를 수정해 JSON을 뽑는다. 에디터(`editor.html`)는 같은 JSON을 만드는 도구이며 생성기와 별도 원본으로 관리하지 않는다.

## 지역 자산 연속성 (맵 확장·분위기 변경 전에)

1. 연결되는 기존 맵의 생성기/JSON과 실제 화면 한 장을 확인하고, 사용할 바닥·경계·원경·주요 소품의 **현재 파일명**을 정한다. 완료 기준은 “같은 지역의 어느 자산을 이어 쓰는지” 설명 가능한 상태다. 색 이름만으로 다른 지역 세트를 고르지 않는다.
2. “공허 느낌·어둡게·차갑게”는 기존 지역 위의 분위기 조절로 적용한다. 지역 자체를 바꾸라는 명시가 없다면 바닥·절벽·배경까지 다른 생물군계로 교체하지 않는다. 새 전용 바닥 요청은 그 부분만 별도 제작한다.
3. 새 맵의 파일명뿐 아니라 실제 사용된 타일 키·backdrop·PNG를 인접 맵과 대조한다. 예: obj는 오브제 지역 기준을 따르며, 단지 보라색이라는 이유로 void의 x/X/y/purple_fire를 복사한 결과는 실패다. 최신 생성본이 있으면 과거 초안이나 폴백으로 되돌리지 않는다.
4. 최종 화면은 인접 맵과 나란히 보며 지역 연속성, 요청한 차이, 배경·둘레가 카메라 안에서 실제 보이는지 확인한다. 테스트 통과·자체 DESIGN 일치만으로 승인하지 않는다. 확인은 바뀐 맵/비트에 한정한다.

사건 근거와 검증 실패: [오브제 지역 자산 포스트모텀](../../../docs/postmortems/2026-09-12-object-region-art.md).

## 출입구·다음 맵 이동 (2026-09-16 BUILD192~194 사용자 지적 세 번)

- 출입구를 벽 타일로 막지 않는다. 통로 타일이 맵 가장자리까지 이어지고, 가장자리 칸은 **걷는 출입구 타일**(용광로 `H`; 맵 밖은 `tileAt` 이 막힌 ' ' 를 돌려줘 떨어지지 않는다).
...
**아직 안 쓴 유형(아이디어)**: 소리 나는 소품(멀어지면 멈춤) · 따라오는 NPC 한 마디 · 발판 밟는 순서 퍼즐 · 시간 제한 통로 · 숨은 통로(나무 사이) · 상자 셋 중 하나만 진짜 · 낚시/던지기 미니게임 · 가게(돈 쓰기 — 돈 시스템 있음) · 보스 전투 기믹 모드(`/enemy`).
**2026-09-12 추가로 쓴 것**: 귀환 발판(동료가 **사라졌다 돌아옴** — `{hide}`/`{show}` + `{aura}`) · 오브젝트 알(조사 → 부들부들 → 쩍 → 소품이 **다리로 도망감**: `{remove}`+`{spawn}`+`{slide}`) · 길을 막는 동상 벽으로 **다른 길로 유도**(막힌 쪽에 이야기 한 덩이) · C 연타 미니게임(`{mash}`) · 로켓 발사(`{fire}`/`{rocket}`).
...
**신경 쓸 것**: 레이아웃 예산 `src/core/layout.js`(대화 중 230px·전투 246px·프로브 19px) 안에서 사각형으로 계산(2026-09-11 레이아웃 포스트모텀 `docs/postmortems/2026-09-11-layout-issues.md`) / 나무·동상 밑동은 항상 땅 타일 위(허공에 뜬 소품 금지 — 청록숲은 `m` 띠) / 스폰은 땅·소품·문 밖 / 문은 양방향 스폰 존재 / 출구는 땅을 화면 끝까지 / 컷신 좌표는 `rel:` / 몹은 `unless:'<맵>_<id>_defeated'` / 브금은 맵 `bgm` 한 줄(컷신 중 껐으면 표준 조우가 복귀시킨다) / **발소리가 있는 바닥**은 타일에 `step:'<sfx>'` · `step:['<sfx>'…]` · 이름 배열(걸음마다 하나를 골라 그대로 재생 — **물걸음 사운드** `water_step01~33`, 목록 `src/data/footsteps.js` 가 그 예. mp3 한 파일의 구간 재생은 브라우저가 `currentTime` 을 무시해 못 쓴다) 만 선언하면 된다(`Player.footstep` 이 80px 마다 재생, 새 sfx 는 `loadSfxFiles` 목록 + `design/audio/references.md`) / 새 타일 글자는 `tiles.js` + `maps.test SOLID_CHARS`·`maps-connect`/`maps-layout` WALK·ROAD 에도 / 새 소품은 `props.test` 키·히트박스 규칙 / 생성기 `--check` 가 `tools/dev/check.sh` 에 잡힘.
...
3. **출입구·문**: 아래 ‘출입구 조작 판정’으로 그림의 열린 상태와 입력을 맞춘다. 전환은 항상 검은 페이드, 보라맵 문은 무음. 언더테일 루인즈처럼 문은 살짝 올라가 있게.
4. **소품·디테일**: 조사 대상으로 정한 소품은 기존 표현과 승인 대사를 연결하고, 무대 장비·난간 같은 비상호작용 장식은 대사를 자동 생성하지 않는다. 지역의 용도를 설명하는 구조물과 이동 여백을 함께 설계한다. 기존 승인 자산을 우선 쓰고 새 그림은 해당 제작 절차로 만든다.
5. **빛·소리·연출**: 색·광원 모양·소리 큐는 해당 장면의 최신 지시가 기준이다. 낙석의 평행광/무음은 낙석 사례이며 모든 무대에 강제하지 않는다. 빛의 가장자리·배우와 배경의 밝기·카메라 이동 중 연속성을 실제 화면으로 확인한다. 자세한 기준은 [연출 판단](../../../docs/development/staging.md)을 따른다.
...
1. 실제 그림에서 문짝이 통로를 막고 있는지 먼저 본다. **열린 아치·뚫린 선체 입구·계단·연속된 길은 `interact:false`**, 닫힌 문짝을 열거나 조사하는 경우만 `interact:true`다. 문 그림 파일의 존재가 입력을 결정하지 않는다.
2. 열린 통로는 출발 스폰에서 방향키만 눌러 문턱을 통과한다. C 없이 검정 전환되고 파티가 다음 맵에 도착해야 통과다. 닫힌 문은 문 앞 정지→C→열림음/전환을 확인한다.
3. 귀환 스폰은 자동 전환 영역 밖에 두고 정지 상태에서 왕복하지 않는지 확인한다. 열린 통로에는 닫힌 문을 여는 철컥 소리를 붙이지 않는다.

근거: [열린 입구와 기존 말풍선 재사용 실패](../../../docs/postmortems/2026-09-13-open-entry-and-bubble.md). BUILD139는 C를 잘 잡는 테스트를 늘렸지만 그림과 입력의 모순을 검증하지 못했다.

## 상호작용 무결성 체크리스트 (RPG 맵에서 항상 터지는 것들 — 전부 엔진/테스트가 막지만, 설계 때도 지킨다)
| 문제 | 규칙 | 어디서 막나 |
|---|---|---|
| **막는 소품(동상 벽 등) 사이로 빠져나감** | 히트박스를 **타일 전체 32×32** 로(받침만 막으면 줄 사이에 12px 틈). 맵 `meta.blocked:[[출발],[막힌 쪽]]` 선언 → 픽셀 단위 플러드필 감사 | `maps-connect.test` |
| **같은 소재의 새 그림이 있는데 옛 그림을 씀** | 쥰희 동상은 PR 포즈 6종(`objlib.JUNHEE_STATUES`), 손그림 `statue_junhee.png` 는 폴백 전용. 새 맵에서 소품을 쓸 때 `assets/source/*` 와 다른 맵이 쓰는 파일명을 먼저 본다 | 리뷰 |
...
3. JSON 작성:
   ...
   - 입력 방식은 ‘출입구 조작 판정’을 따른다. 닫힌 문의 C 영역은 문 아래 접근로에 닿게 넓힌다(예 `void4` door_zone `x:2336,y:206,w:72,h:26`). 잠긴 문은 합류 전에도 C 로 "잠겨 있다" 가 떠야 한다.

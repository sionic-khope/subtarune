...
3. JSON 작성:
   ...
   - 위험 구간 맵은 **화면 두세 배 이상** 길게(void5 30열·void6 48열·void7 60열). 짧으면 '들어가자마자' 느낌이 난다. 낙석 맵은 손으로 쓰지 말고 `/usr/bin/python3 tools/maps/rockfall_map.py <id> --rocks n --entry top|left --exit down|right --prev … --next …` 로 뽑는다(`--check` 로 기존 맵 재현 확인).

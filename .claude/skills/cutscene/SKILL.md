...
등장·기습·화면 밖 진입을 만들 때는 **등장 전에는 안 보임 → 지정 방향으로 들어오는 중 → 다음 대사 전에 도착**을 카메라·줌이 적용된 화면에서 확인한다. 맵 좌표상 왼쪽이라는 것만으로 화면 밖이라고 판단하지 않는다. 필요하면 초기 hidden과 진입 시 show를 함께 사용한다.

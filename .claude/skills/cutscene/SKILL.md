...
- **fling 소리는 장면마다 다르게**(2026-09-15 사용자 피드백 2 “왤캐 재사용하노”): `sfx:'whoosh'`를 기본값으로 쓰지 않는다. 이미 쓴 것 — 파크가디언 박치기 `fling_whistle`, 비데→마리오 `cannon_puff`. 새 fling 은 새 소리를 만들거나 안 쓴 소리를 고르고 `design/audio/references.md` 에 적는다. 날리는 타이밍은 원인 동작(박치기 등) 직후 **즉시**(쿵·흔들림과 같은 `parallel`), 사이에 `wait` 를 두지 않는다.

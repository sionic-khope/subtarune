// 오디오 참조 감사: 코드·맵·컷신이 부르는 sfx/bgm/voice 이름이 실제 파일(assets/audio/*) 또는 합성 폴백(audio.js case)에 있어야 한다.
...
const SYNTH = new Set([...audioSrc.matchAll(/case '([a-z_]+)':/g)].map((m) => m[1]));
const VOICE_PRESETS = new Set([...(audioSrc.match(/VOICES\s*=\s*\{([\s\S]*?)\n\};/)?.[1] || '').matchAll(/^\s*([a-z_]+)\s*:/gm)].map((m) => m[1]));
...
for (const p of sources) {
  ...
  for (const m of s.matchAll(/\bsfx\s*:\s*'([a-z_]+)'|\.sfx\(\s*'([a-z_]+)'|sfx\(\s*'([a-z_]+)'/g)) add('sfx', m[1] || m[2] || m[3], p);
  for (const m of s.matchAll(/\bbgm\s*:\s*'([a-z_]+)'|"bgm"\s*:\s*"([a-z_]+)"|playBgm\(\s*'([a-z_]+)'|preloadBgm\(\s*'([a-z_]+)'/g)) add('bgm', m[1] || m[2] || m[3] || m[4], p);
  for (const m of s.matchAll(/\bvoice\s*:\s*'([a-z_]+)'|blip\(\s*'([a-z_]+)'/g)) add('voice', m[1] || m[2], p);
}
...
test('test_audio_loadSfxFiles_list_matches_files', () => {
  ...
  const list = [...(main.match(/loadSfxFiles\(\[([^\]]*)\]/)?.[1] || '').matchAll(/'([a-z_]+)'/g)].map((m) => m[1]);

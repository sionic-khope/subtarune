...
test('test_night_cliff_preserves_exact_dialogue_order_and_censors_only_requested_word', () => {
  ...
  assert.deepEqual(scene.filter(n => n.text).map(n => [n.speaker, n.text.slice(2)]), [
  ...
  for (const n of scene.filter(n => n.text)) assert.equal(n.voice, n.speaker === '경섭' ? 'gyeongsub' : 'choimis');
});
...
test('test_old_item_save_resumes_new_view_without_repeating_reward', () => {
  ...
  assert.equal(scene.find(n => n.map === 'jjajang_sakura12').spawn, 'from_scene');
});

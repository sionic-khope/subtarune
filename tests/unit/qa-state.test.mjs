...
test('test_qa_state_saloon_and_later_points_include_purchased_upgrades_and_cost', () => {
  const saloonIndex = QA_POINTS.findIndex((point) => point.id === 'maillard_saloon');
  assert.ok(saloonIndex >= 0);
  for (const point of QA_POINTS.slice(saloonIndex)) {
...
test('test_qa_state_before_saloon_keeps_optional_shop_upgrades_unpurchased', () => {
  const saloonIndex = QA_POINTS.findIndex((point) => point.id === 'maillard_saloon');
  assert.ok(saloonIndex >= 0);
  for (const point of QA_POINTS.slice(0, saloonIndex)) {
...
test('test_castle_qa_includes_consumed_rescue_upgrades_once_with_full_maximum_hp', () => {
  const points = QA_POINTS.filter(point => /^gajaeman_(castle|memory|regret|torii)/.test(point.map));
...
test('test_precastle_qa_preserves_optional_rescue_shop_purchase_and_immediate_consumption', () => {
  for (const point of QA_POINTS.filter(point => !/^gajaeman_(castle|memory|regret|torii)|^ship_(lounge|deck)_epilogue$/.test(point.map || ''))) {

...
const st = () => page.evaluate(() => ({ map: game.mapId, ride: !!game.ride, p: [Math.round(game.player.x), Math.round(game.player.y)], running: game.dialogue.running, box: game.textbox.state, speaker: game.textbox.speaker, text: game.textbox.node?.text || '', zoom: +game.zoom.s.toFixed(2), cam: [Math.round(game.camera.x), Math.round(game.camera.y)], locked: game.camera.locked, flags: { ...game.flags }, sfx: window.__sfx || [], row5: game.map.rows[5].slice(56, 60), bridgeFall: !!game.entities.find((e) => e.id === 'bridge_fall' && !e.dead), leverOn: !!game.entities.find((e) => e.id === 'lever_on' && !e.dead && e.visible) }));
...
// 뗏목 타기 (길게)
await stand(132, 210, 'right'); await page.waitForTimeout(120);
const t0 = Date.now(); await page.keyboard.press('KeyC'); await page.waitForTimeout(300);
s = await st(); check('boards long raft', s.ride);
await page.waitForTimeout(8000); await page.screenshot({ path: `${S}/void4_02_mid.png` });
...
check('ride ~18s', rideSec > 15 && rideSec < 24, rideSec.toFixed(1) + 's');
// 도착 컷신
...
check('ends with narrator 오케이', seen.some((x) => x.startsWith('|* 오케이')), seen.join(' / '));
s = await st(); check('camera back to player, zoom 1', !s.locked && s.zoom === 1);
// 잠긴 문
await stand(3520, 230, 'right'); await page.waitForTimeout(120); await page.keyboard.press('KeyC'); await page.waitForTimeout(300);
s = await st(); check('door locked line', s.text.includes('자물쇠'), s.text); await finish();
// 계단 → 레버
await stand(3460, 300, 'up'); await page.keyboard.down('ArrowUp'); await page.waitForTimeout(400); await page.keyboard.up('ArrowUp');
await page.evaluate(() => { game.player.x = 3460; game.player.y = 120; }); await page.keyboard.down('ArrowUp'); await page.waitForTimeout(1200); await page.keyboard.up('ArrowUp'); await page.waitForTimeout(100);
s = await st(); check('climbed stairs to upper platform', s.p[1] < 96, JSON.stringify(s.p));
await stand(3492, 66, 'up'); await page.waitForTimeout(120); await hookSfx();
...
await stand(3380, 168, 'left'); await page.keyboard.down('ArrowLeft'); await page.waitForTimeout(1500); await page.keyboard.up('ArrowLeft');
s = await st(); check('can walk on bridge tiles', s.p[0] < 3100 && s.p[1] >= 160 && s.p[1] < 192, JSON.stringify(s.p));
await stand(1900, 168, 'left'); await page.keyboard.down('ArrowLeft'); await page.waitForTimeout(900); await page.keyboard.up('ArrowLeft'); await page.waitForTimeout(120);
s = await st(); check('stopped next to pillar', s.p[0] >= 1792 && s.p[0] <= 1800, JSON.stringify(s.p));

...
try {
  ...
  await key('KeyX');
  await page.evaluate(() => { game.money = 2800; game.setFlag('shop_yongjun_greeted'); });
  await walk(1084, 364); await key('ArrowUp');
  await snap('01-shop-front');
  await key('KeyC');
  await page.waitForFunction(() => game.state === 'shop' && game.shop.art && !game.transitioning && !game.shop.waitForRelease && game.shop.lock === 0);
  ...
  check('purchase autosaves while shop remains open', saved.money === 2630 && saved.attack === 3 && saved.hpBonus === 40 && saved.flags.shop_yongjun_cialis && saved.flags.shop_yongjun_vaseline);
  await key('KeyC');
  check('sold-out cannot charge twice', await page.evaluate(() => game.shop.message.reason === 'sold_out' && game.money === 2630));
  await snap('04-sold-out'); await key('KeyX'); await key('KeyX'); await key('KeyX');
  await page.evaluate(() => game.continueGame());
  await page.waitForFunction(() => !game.transitioning && game.fade.alpha === 0);
  check('continue restores totals without reapplying upgrades', await page.evaluate(() => game.attack === 3 && game.hpBonus === 40 && game.money === 2630));

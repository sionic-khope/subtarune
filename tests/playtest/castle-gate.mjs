...
await runScenario({ name: 'castle-gate', launchOptions: { args: ['--autoplay-policy=no-user-gesture-required'] } }, async ({ page, open, until, press, shot, fixture, check }) => {
  ...
  const observe = async () => fixture('production-call-observer', 'Read-only wrappers record actual sound calls, actor draw alpha, completed draw states and dialogue. No positions, flags, timings or inputs are injected.', () => {
    const q = window.__gateQA = { sounds: [], walks: [], alpha: [], samples: [], transits: [], texts: [] };
    ...
    game.draw = (...args) => {
      ...
      for (const passenger of scene?.passengers || []) if (!q.transits.includes(passenger.actor.id)) q.transits.push(passenger.actor.id);
  ...
  if (mode === 'cancel') {
    ...
    check('boulder return original three lines occur before reunion', JSON.stringify(q.texts.slice(0, 3).map(t => t.text)) === JSON.stringify(['* 다 됐노?', '* 이제 빨리 다시 가운데 맵으로 가볼까', '* ㅇㅋ요']));
    ...
    check('six allies actually traverse door in order', q.transits.join() === 'gate_youngcle,gate_junhee,gate_bidet,gate_mario,gate_ttuulla,gate_park', JSON.stringify(q.transits));

...
check('splash while moving (board + periodic + arrive)', r.splashes >= 5, String(r.splashes));
...
await stand(1000, 420, 'right'); await page.keyboard.down('ArrowRight'); await page.waitForTimeout(1200); await page.keyboard.up('ArrowRight'); await page.waitForTimeout(200);
await page.keyboard.press('KeyC'); await page.waitForTimeout(300);
s = await st(); check('exit door reachable', s.text.includes('검은 문'), JSON.stringify({ p: s.p, text: s.text }));
await page.screenshot({ path: `${S}/void3_04_exit.png` });
await finish();

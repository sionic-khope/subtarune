// 벚꽃 숲(jjajang_sakura, BUILD261): QA jjajang_sakura 에서 브금 sakura·꽃잎 조금·발소리 없음 → 위로 걸어(달리지 않고) 넓은 풀숲 초입 트리거 → 꽃잎 폭발·땅과 나무가 번지듯 분홍으로 → 위로 → 오른쪽으로 꺾어 끝까지 → 재진입 땐 처음부터 핀 상태. 실행: tests/playtest/run.sh jjajang-sakura
...
try {
  ...
  check(s.groundRows === 56 && s.pinkRows === 0 && s.bloomedTrees === 0, `아직 검은 풀숲 땅·어두운 나무 ${JSON.stringify([s.groundRows, s.pinkRows, s.bloomedTrees, s.trees])}`);
  ...
  const t0 = Date.now();   // 기본이 달리기(≈220px/s), X 를 누르면 천천히 걷기(÷1.75) — “8초정도 걷다보면” 은 천천히 걷기 기준(28행 ≈ 7초)
  check(await go('ArrowUp', 'g.flags.sakura_bloom', 30000, true), '위로 걸어 넓은 풀숲 초입 → 벚꽃이 번지기 시작(sakura_bloom)');
  const walked = (Date.now() - t0 + 700 * 1.75) / 1000; check(walked > 5 && walked < 12, `입구에서 천천히 걸어 ${walked.toFixed(1)}초(8초쯤)`);
  await page.waitForTimeout(250); s = await state(); await cap('01_burst');
  check(s.petals > 150 && s.spread, `꽃잎 폭발(${s.petals}개)·번짐 진행 중`);
  check(!(await ev(() => window.game.dialogue.running)), '멈춤 없이 걸을 수 있다');
  await page.waitForTimeout(1200); s = await state(); await cap('02_spreading');
  ...
  check(s.groundRows === 0 && s.pinkRows === 56 && s.bloomedTrees === s.trees, `땅 전부 분홍·나무 전부 벚꽃 ${JSON.stringify([s.groundRows, s.pinkRows, s.bloomedTrees, s.trees])}`);
  ...
  check(await until(() => window.game.mapId === 'jjajang_sakura' && !window.game.transitioning && window.game.player.y > 60 * 32, 15000), '입구로 재진입');
  await page.waitForTimeout(800); s = await state(); await cap('05_reentry');
  check(s.groundRows === 0 && s.pinkRows === 56 && s.bloomedTrees === s.trees && s.rate === 18 && !s.spread, `재진입도 핀 상태 ${JSON.stringify([s.groundRows, s.pinkRows, s.bloomedTrees, s.rate])}`);
  // 공터 위 문 ↔ 벚꽃 숲 연결

// 벚꽃 숲(jjajang_sakura, BUILD261~262): QA jjajang_sakura 에서 브금 sakura·꽃잎 조금·발소리 없음 → 위로 달려 넓은 풀숲 초입 트리거(예약) → 브금 하이라이트 7.6초에 거대 벚꽃 대각선 sweep·꽃잎 폭발·땅과 나무가 번지듯 분홍으로 → 위로 → 오른쪽으로 꺾어 끝까지 → 재진입 땐 처음부터 핀 상태. 실행: tests/playtest/run.sh jjajang-sakura
...
try {
  ...
  check(s.groundRows === 68 && s.pinkRows === 0 && s.bloomedTrees === 0, `아직 검은 풀숲 땅·어두운 나무 ${JSON.stringify([s.groundRows, s.pinkRows, s.bloomedTrees, s.trees])}`);
  ...
  // 기본이 달리기(≈220px/s): 40행 길을 달리면 브금 하이라이트(7.6초) 전에 풀숲 초입에 닿아 예약(bloomArmed) → 하이라이트 순간에 거대 벚꽃 sweep + 번짐(사용자 “몇초 걷다가 브금 하이라이트때 쫙”)
  ...
  await page.waitForTimeout(380); s = await state(); await cap('01_sweep');
  ...
  check(mid.t > 0.2 && mid.t < 0.8 && mid.pinkPixels > 0.03, `sweep 중간(0.4초)에 큰 벚꽃이 화면에 있다 ${JSON.stringify(mid)}`);
  check(s.petals > 150 && s.spread, `꽃잎 폭발(${s.petals}개)·번짐 진행 중`);
  check(await until(() => !window.game.sweep, 4000), 'sweep 은 1초(+꼬리) 뒤 끝');
  ...
  check(s.groundRows === 0 && s.pinkRows === 68 && s.bloomedTrees === s.trees, `땅 전부 분홍·나무 전부 벚꽃 ${JSON.stringify([s.groundRows, s.pinkRows, s.bloomedTrees, s.trees])}`);
  ...
  check(await until(() => window.game.mapId === 'jjajang_sakura' && !window.game.transitioning && window.game.player.y > 70 * 32, 15000), '입구로 재진입');
  await page.waitForTimeout(800); s = await state(); await cap('05_reentry');
  check(s.groundRows === 0 && s.pinkRows === 68 && s.bloomedTrees === s.trees && s.rate === 18 && !s.spread, `재진입도 핀 상태 ${JSON.stringify([s.groundRows, s.pinkRows, s.bloomedTrees, s.rate])}`);
  // 공터 위 문 ↔ 벚꽃 숲 연결

...
let flames0 = 0, flamesMax = 0; let bgmAtReady = 'x', bgmAtMash = 'x', mashSeen = false, mashMax = 0, mashDone = false, lineup = null, fireSeen = false, bgmAtLaunch = 'x', launchGone = false, camFollowedRocket = false, boomDelay = null, tLaunch = 0;
const lines3 = await pump(240000, async (k) => {
  ...
  if (k.includes('뜨거운')) { await page.waitForTimeout(300); fireSeen = await page.evaluate(() => game.flames.length > 0 && game.flameEmitters.length > 0); flames0 = await page.evaluate(() => game.flames.length); await page.screenshot({ path: `${S}/obj1_07_fire.png` });
...
check('fire starts small and builds up (flames grow ≥ 2.5× while he talks), the line is CUT mid-sentence → launch: bgm off, rocket sfx, camera follows the cannon with a flame trail, both gone off the map; boom ≈ 3s later', fireSeen && flamesMax >= flames0 * 2.5 && flamesMax >= 40 && launchGone && bgmAtLaunch === null && s.sfx.includes('rocket') && camFollowedRocket && boomDelay !== null && boomDelay >= 2.5 && boomDelay <= 4.2 && s.sfx.includes('boom'), JSON.stringify({ fireSeen, flames0, flamesMax, launchGone, bgmAtLaunch, camFollowedRocket, boomDelay }));

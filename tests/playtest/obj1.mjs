...
check('each shout pushes the cannon one tile (32px) with 드륵 (scrape ×5), and the pushers walk with it (leg frames change)', s.cannon.x === meta.cannon[0] + 34 + 5 * 32 && s.junhee.x === meta.pushers[0] + 5 * 32 && s.yongjun.x === meta.pushers[2] + 5 * 32 && s.sfx.filter((n) => n === 'scrape').length === 5 && junheeWalked, JSON.stringify({ cannon: s.cannon, junhee: s.junhee, yongjun: s.yongjun, scrape: s.sfx.filter((n) => n === 'scrape').length, junheeWalked, cannonXs }));
...
check('meeting: 쥰희 "!" emote, drumroll then cannon close-up (zoom > 1.5) on the name line, fanfare after it', emoteSeen && s.sfx.includes('drumroll') && zoomCannon >= 1.5 && s.sfx.includes('fanfare') && bgmAtName === 'vs_lancer', JSON.stringify({ emoteSeen, zoomCannon, bgmAtName, sfx: [...new Set(s.sfx)] }));

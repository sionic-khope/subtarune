...
check('용준 becomes the after-entity right away (no map reload needed) and stands on the row you walk in on', !!yj && yj.y === 264, JSON.stringify(yj));
// 왼쪽 문 스폰 줄에서 오른쪽으로 걸어가면 프로브에 잡힌다(아랫줄에 있으면 지나쳐 버린다 — 2026-09-11 버그)
const sp = await page.evaluate(async () => (await import('/src/data/maps.js')).MAPS.obj1.spawns.from_left);
await stand(yj.x - 300, sp.y, 'right');
let reached = null; for (let i = 0; i < 60; i++) { await page.keyboard.down('ArrowRight'); await page.waitForTimeout(60); await page.keyboard.up('ArrowRight'); reached = await page.evaluate(() => { const t = game.player.probe(); return t ? t.id : null; }); if (reached) break; }
check('walking right along the entry row bumps into 용준 (probe hits him, not past him)', reached === 'yongjun_after', JSON.stringify({ reached, playerY: sp.y, yj }));
await page.keyboard.press('KeyC'); await page.waitForTimeout(300);
let flames0 = 0, flamesMax = 0, grow = 0; let bgmAtReady = 'x', bgmAtMash = 'x', mashSeen = false, mashMax = 0, mashDone = false, lineup = null, fireSeen = false, bgmAtLaunch = 'x', launchGone = false, camFollowedRocket = false, boomDelay = null, tLaunch = 0;
const lines3 = await pump(240000, async (k) => {
  ...
  if (k.includes('준비')) { await page.waitForTimeout(150); bgmAtReady = (await st()).bgm; await advance(); }   // 줄이 뜨는 순간 브금이 꺼져 있어야(페이드 0)
  ...
  if (k.includes('뜨거운')) { await page.waitForTimeout(300); fireSeen = await page.evaluate(() => game.flames.length > 0 && game.flameEmitters.length > 0); flames0 = await page.evaluate(() => game.flames.length); grow = await page.evaluate(() => (game.flameEmitters[0] || {}).grow ?? 0); await page.screenshot({ path: `${S}/obj1_07_fire.png` });
    for (let i = 0; i < 24; i++) { await page.waitForTimeout(100); const f = await page.evaluate(() => game.flames.length); if (f > flamesMax) flamesMax = f; if (await page.evaluate(() => !game.textbox.node || game.textbox.state === 'closed')) break; }   // 끊길 때까지(대사 중 불이 점점 세짐)
    tLaunch = Date.now();
...
const want3 = ['박용준|* 형들 자 제 뒤에 이렇게 딱 맞춰서 서봐요', '박용준|* 그리고... 준비 ~~~~~~~~~~~~~~~', '박용준|* 밀어!!!!!!!!!!!!!!!!!!!!!', '박용준|* 어어어.. 어?', '박용준|* 뭐 뭐지 뭔가 등이... 뜨거운 느끼..', '경섭|* 허허 가버렸네', '억빠맨|* 신경쓰지말고 갈길가죠. 근데 그것이 도대체 뭘까요?', '경섭|* 허허 그러게', '억빠맨|* 뭐냐고 씨발년아', '경섭|* 응?', '억빠맨|* 아니에요 가시죠'];
check('push scene: 11 lines verbatim in order (the duplicate 그리고 line is gone)', JSON.stringify(lines3) === JSON.stringify(want3), JSON.stringify(lines3));
...
check('fire starts small and builds up (emitter grow ≥ 2, particles ≥ 1.6× and ≥ 30 while he talks), the line is CUT mid-sentence → launch: bgm off, rocket sfx, camera follows the cannon with a flame trail, both gone off the map; boom ≈ 3s later', fireSeen && grow >= 2 && flamesMax >= flames0 * 1.6 && flamesMax >= 30 && launchGone && bgmAtLaunch === null && s.sfx.includes('rocket') && camFollowedRocket && boomDelay !== null && boomDelay >= 2.5 && boomDelay <= 4.2 && s.sfx.includes('boom'), JSON.stringify({ fireSeen, grow, flames0, flamesMax, launchGone, bgmAtLaunch, camFollowedRocket, boomDelay }));

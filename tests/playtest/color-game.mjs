...
try {
  ...
  const around = await g(); check(around.junhee.facing === 'up' && Math.abs(around.junhee.x - 383) < 120 && Math.abs(around.px - 480) < 140, '모두 TV 아래 모여 위를 본다 ' + JSON.stringify([around.junhee, around.px, around.py]));
  await cap('gather');

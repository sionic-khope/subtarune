...
await runScenario({ name: 'choimis-301', launchOptions: { args: ['--autoplay-policy=no-user-gesture-required'] } }, async ({ page, open, until, press, shot, fixture, check }) => {
  ...
  const cases = scope === 'all' ? ['idle', 'kart', 'blood-first', 'blood-later', 'gasuni-normal', 'gasuni-charge', 'ordinary-choso', 'eating'] : [scope];
  ...
  const save = () => fs.writeFileSync(path.join(process.env.SHOT_DIR, 'build301-evidence.json'), JSON.stringify(evidence, null, 2) + '\n');
  ...
  await fixture('registered-round-observer', 'Skip already-covered opening and activate its defense state. Read full Game.draw, damage/audio and real snapshots; do not alter results or combat time.', () => {
    ...
    const q = window.__qa301 = { name: 'idle', samples: [], frames: {}, sounds: [], hurts: [], sequence: [] };
    const e = b.enemies[0];
    ...
    game.sound.sfx = (name, options) => {
      const handle = sfx(name, options), entry = { name, options, at: performance.now(), samples: [] }; q.sounds.push(entry);
    ...
    game.draw = (...args) => {
      ...
      if (!q.samples.length || now - q.samples.at(-1).at > 65) q.samples.push({ at: now, state: b.state, mode: b.activeEnemyMode, snapshot: s, hp: e.hp, party: b.members.map(m => ({ hp: m.hp, down: m.down })), bubble: b.bubble?.text, enemy: { x: e.x, y: e.y, scale: e.def.scale, scaleY: e.def.scaleY, pivot: e.def.pivot } });
  ...
  for (const name of cases.filter(name => name !== 'idle')) {
    const type = name === 'kart' ? 'choimis_pink_kart' : name.startsWith('blood') ? 'choimis_pink_choso' : name.startsWith('gasuni') ? 'choimis_pink_gasuni' : name === 'ordinary-choso' ? 'choimis_choso' : 'choimis_eating_race';
    const selected = await fixture(`select-${name}`, 'Resolve the requested type/cycle through the actual selector and restore normal HP between rounds. No projectile, hit, elapsed-time or outcome state is injected.', ({ name, type }) => {
      ...
      Object.assign(window.__qa301, { name, samples: [], frames: {}, sounds: [], hurts: [] });
    ...
    const start = Date.now(); let held = null, chargeHeld = false, fired = 0, lastFire = 0;

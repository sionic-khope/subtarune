...
{ const lay = await page.evaluate(() => ({ ys: game.battle.members.map((m) => m.home[1]), poses: game.battle.members.map((m) => m.pose), soulR: game.battle.soul.r }));
  check('party fits above the panel at y 246 (feet y ≤ 220, spacing ~75px) and does the attack pose at intro', lay.ys.every((y) => y <= 220 && y >= 60) && lay.ys[1] - lay.ys[0] <= 80 && lay.poses.some((p) => p !== null && p !== undefined), JSON.stringify(lay));
...
check('flavor line ("* 억빠맨이 CS 막타를…" 류) coexists with [공격하기][아이템] on the action-select screen', !!b && typeof b.text === 'string' && b.text.startsWith('* '), JSON.stringify({ text: b?.text }));

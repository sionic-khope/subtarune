...
// BUILD359: 뗏목 점프 → 곡(원곡 42.7초부터) → 화면 전체 상승(성벽 → 노을 바다, 슬로우모션) → 원곡 58초 흰 번쩍임·노을 땅 → 가재맨 도망 → 앞덤블링 → 원곡 64초 무릎 착지(챱).
await runScenario({ name: 'castle-rise', launchOptions: { args: ['--autoplay-policy=no-user-gesture-required'] } }, async ({ page, open, until, shot, check }) => {
  ...
  check('rise track plays', end.bgm === 'save_the_world_rise', end.bgm);
});

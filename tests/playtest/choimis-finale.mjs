...
await runScenario({ name: 'choimis-finale' }, async ({ page, open, until, press, shot, check, fixture }) => {
  ...
  await fixture('lethal-action-boundary', 'Skip already-covered opening, preserve existing legitimate party stats, prepare boss HP 1 and defense boost before the real three-party attack queue. Loss-only scope additionally prepares party HP 1 before any fight; actual hazards must cause defeat. Observe Game.draw only after its full original draw returns.', lossOnly => {
    ...
    const q = window.__finalQa = { frames: {}, phases: [], samples: [], texts: [], audio: [], initialParty: b.members.map(m => ({ id: m.id, hp: m.hp, maxHp: m.maxHp })), canvas: [game.canvas.width, game.canvas.height] };
    const draw = game.draw.bind(game), sound = game.sound.sfx.bind(game.sound), stopBgm = game.sound.stopBgm.bind(game.sound);
    game.sound.sfx = (name, options) => { const result = sound(name, options); q.audio.push({ type: 'sfx', name, options, at: performance.now(), phase: b.gimmick?.snapshot?.phase }); return result; };
    ...
    game.draw = (...args) => {
      ...
      if (!q.samples.length || now - q.samples.at(-1).at > 100) q.samples.push({ at: now, phase: s.phase, phaseTime: s.phaseTime, charge: s.chargeProgress, assault: s.assault ? { elapsed: s.assault.elapsed, stage: s.assault.stage, heart: s.assault.heart, contacts: s.assault.contacts, destroyed: s.assault.destroyed, spawned: s.assault.spawned, shots: s.assault.shots.length, hazards: s.assault.hazards.length } : null, boss: s.boss, normal: s.normal, party: b.members.map(m => ({ hp: m.hp, maxHp: m.maxHp, down: m.down })) });
      ...
      if (!s.phase.endsWith('talk') && s.phase !== 'assault' && s.phaseTime >= (s.phase === 'flash' ? 0.06 : 0.12)) capture(s.phase);
      ...
      if (s.phase === 'shot' && s.phaseTime >= 0.5) capture('beam-large');
  ...
  while (Date.now() - wallStart < 85000) {
    ...
    const delta = state.s.assault.safeY - state.s.assault.heart.y;
    ...
    held = key;
    if (shots++ % 5 === 0) await press('KeyC', { delay: 20 });
    await page.waitForTimeout(70);
  }
  if (held) await page.keyboard.up(held);
  evidence.survivalWallMs = Date.now() - wallStart;
  check(captureOnly ? 'disclosed capture fixture reaches real defeat dialogue' : 'real sixty-second assault survives into defeat dialogue', await until(() => game.battle?.gimmick?.snapshot?.phase === 'death-talk', 4000));
  ...
  if (!captureOnly) check('assault real wall duration reaches sixty seconds', phase('bursts')?.at - phase('assault')?.at >= 59500, String(phase('bursts')?.at - phase('assault')?.at));
  ...
  if (!captureOnly) criticalFrames.push('assault-0.3', 'assault-31', 'assault-59');

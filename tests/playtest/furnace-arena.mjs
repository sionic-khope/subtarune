...
try {
  ...
  s = await untilText('반갑노'); check(!!s && s.tv.phase === 'on' && s.tv.expr === 'greet' && s.tv.fold === 1 && s.tv.x === 300 && s.tv.y === 100 && s.bgm.includes('storage_show') && s.camx >= 250 && s.camx <= 300, 'TV 가 일행 앞 가운데에서 펼쳐져 켜지고 “반갑노 게이들아”(인사) + 영클 브금(카메라 가운데) ' + JSON.stringify([s?.tv, s?.bgm]));
  await cap('greet');
  ...
  s = await untilText('살려줘요홍홍'); check(!!s && s.tv.x === 300 && s.tv.x + 236 <= s.cage.x && s.tv.phase === 'on' && s.tv.expr === 'taunt' && s.tv.fold === 1, 'TV 가 철창 왼쪽 옆으로 내려와 펼쳐져 조롱(철창을 가리지 않음) ' + JSON.stringify([s?.tv, s?.cage])); await cap('tv_beside_cage');
  ...
  s = await untilText('조작 패널이 보이는가'); check(!!s && s.tv.x === 300 && s.tv.y === 100 && s.tv.fold === 1 && s.camx >= 250 && s.camx <= 300, 'TV 가 접혔다 일행 앞으로 다시 내려와 규칙 설명(카메라 가운데) ' + JSON.stringify([s?.tv, s?.camx])); await cap('rules');
  ...
  check(!s.dialogue && s.flag && s.tv.phase === 'on' && s.tv.expr === 'smirk' && s.tv.x === 690 && !overlap && s.cage.visible && s.bgm.includes('storage_show'), '연출 끝: TV 는 오른쪽 끝에서 팔짱 끼고 켜진 채 대기(철창을 가리지 않음), 철창 그대로, 영클 브금, 조작 복귀 ' + JSON.stringify([s.flag, s.tv, s.cage.visible, overlap]));

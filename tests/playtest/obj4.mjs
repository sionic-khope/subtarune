...
try {
  ...
  await page.evaluate(() => {
    window.baronEvidence = { roar: new Set(), emerge: [], flies: {}, sounds: [], puff: false, grubClose: false };
  ...
  while (Date.now() - start < 150000) {
    const s = await page.evaluate(() => ({ running: game.dialogue.running, state: game.textbox.state, text: game.textbox.node?.text || '', auto: game.textbox.auto,
  ...
  check('cutscene ends with completion flag and no battle', result.done && !result.battle && !result.running && result.zoom === 1, JSON.stringify(result));
  ...
  await page.screenshot({ path: `${shots}/obj4_end.png` });
  await page.evaluate(() => game.changeMap('obj4', 'from_bottom', true));
  await page.waitForTimeout(500);
  const reentry = await page.evaluate(() => ({ running: game.dialogue.running, actors: game.entities.filter((e) => !e.dead).map((e) => e.id), done: game.flags.obj4_baron_done }));
  check('reentry retains Baron without replaying or resurrecting cannon/Yongjun/grub', reentry.done && !reentry.running && reentry.actors.includes('baron_after') && !['yongjun', 'cannon_up', 'voidgrub'].some((id) => reentry.actors.includes(id)), JSON.stringify(reentry));

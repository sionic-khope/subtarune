// 보라맵11 거대 나무 검증: ?qa=void11 → 조용히 시작(브금 없음), 형섭이 먼저 보이고 카메라가 오른쪽으로 → 쥰희 첫 대사에 Lancer → 대사 순서(브리핑 그대로, 30줄) → 식은땀 → '우리들의 월드' 뒤 웃음 → 눈앞까지 다가감(겹침 없음)+경섭 식은땀 → 통나무 뺏고 오른쪽으로 사라짐
...
const r = await drain(90000, async (q) => {
  ...
  return { bgm: q.bgm, text: q.text.slice(0, 12), jm: !!q.junhee?.motion, je: q.junhee?.emote || null, ge: q.gyeongsub?.emote || null, jdead: q.junhee ? q.junhee.dead : 'gone', jx: q.junhee?.x ?? null, logs: q.logs ? q.logs.dead : 'gone', cam: q.cam.x, p: q.p[0], gf: q.gyeongsub?.facing || null, gap: q.junhee && q.gyeongsub && !q.junhee.dead ? Math.hypot(q.junhee.x - q.gyeongsub.x, q.junhee.y - q.gyeongsub.y) : null };
});
const L = r.lines;
const ORDER = ['쥰희|* 형 빨리 형 옮겨야해', '경섭|* 헉 헉 헉.. 나무 다 캤어.', '쥰희|* 아 진짜!! 빨리좀 해 느려 터졌네', '경섭|* 미안해', '쥰희|* 형이 그래서 안되는거야', '경섭|* 응', '쥰희|* 1500 뜯긴새끼 ㅋㅋ', '경섭|* 야.', '쥰희|* 어 미안 장난이였는데',
  '경섭|* 어쨋든 조금만 쉬면 안될까 형 너무 힘들다', '쥰희|* 형 이제 코앞이야 우리들의 세계 우리들의 월드', '쥰희|* 더러운 김형섭따위 없는 세상이 왔어.', '쥰희|* 나의 마이야르 슈퍼 페이스츄리 어쩌고 타코마스터 세상이 온거라고!!!!!', '경섭|* 허허', '쥰희|* 그렇기에',
...
check('all 30 lines in briefing order (no "테스트룸에 참고" — that was an instruction, not a line)', !L.some((l) => l.includes('테스트룸')) && idx.every((i) => i >= 0) && idx.every((v, i) => i === 0 || v > idx[i - 1]), JSON.stringify(ORDER.filter((k, i) => idx[i] < 0)) + ' got=' + JSON.stringify(L));
...
{ const o = r.obs; const firstLine = o.findIndex((x) => x.text.startsWith('* 형 빨리')), firstLancer = o.findIndex((x) => x.bgm === 'lancer');
  ...
  check('arrival: player framed first (cam < 200), then the camera pans right to the pair (cam > 500) before the first line', o[0].cam < 200 && o.slice(0, Math.max(1, firstLine)).some((x) => x.cam > 500), JSON.stringify({ cam0: o[0].cam, firstLine }));
  ...
  check('laugh motion after "우리들의 월드" and before "더러운 김형섭", and again before "으하하"', laughI >= 0 && laughI >= lineI('* 형 이제 코앞이야') && laughI <= lineI('* 더러운') && o.slice(lineI('* 응 그럴게')).some((x) => x.jm), JSON.stringify({ laughI, world: lineI('* 형 이제 코앞이야'), dirty: lineI('* 더러운') }));
  { const near = o.slice(lineI('* 그렇기에'), lineI('* 응 그럴게') + 1).filter((x) => x.gap !== null); const minGap = Math.min(...near.map((x) => x.gap)); check('approach: junhee stops right in front of gyeongsub without overlapping (gap 60~90px at "그것")', o[lineI('* 우리는 빨리')]?.gap >= 60 && o[lineI('* 우리는 빨리')]?.gap <= 90, JSON.stringify({ gapAtLine: o[lineI('* 우리는 빨리')]?.gap, minGap })); }
  check('gyeongsub sweats when junhee gets close', o.slice(lineI('* 그렇기에'), lineI('* 응 그럴게') + 1).some((x) => x.ge === 'sweat'), '');
...
{ const tr = await page.evaluate(() => { const t = game.entities.find((e) => e.id === 'tree'); return { x: t.x, y: t.y, w: t.w, h: t.h }; });
  await stand(tr.x + tr.w / 2 - 12, tr.y + tr.h + 6, 'up'); await page.waitForTimeout(250); await page.keyboard.press('KeyC'); const t = await drain(10000);

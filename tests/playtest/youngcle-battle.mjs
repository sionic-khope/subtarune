...
const seen = {}, newShot = {};   // BUILD213 새 패턴 3종: 턴별로 본 탄 모양·첫 프레임
...
const untilMenu = async (ms = 60000) => { const t0 = Date.now(); while (Date.now() - t0 < ms) { const s = await st(); if (!s) return null;
    if (s.state === 'bullets' && s.current && s.shapes?.length) { seen[s.current] = seen[s.current] || new Set(); for (const sh of s.shapes) seen[s.current].add(sh); const want = { obangsun_beam: 'obeam', naram_tank: 'shell', youngcle_ship: 'warship' }[s.current]; if (want && !newShot[s.current] && s.shapes.includes(want)) { newShot[s.current] = true; await page.waitForTimeout(250); await cap('21_' + s.current); } } if (godMode && (s.state === 'bullets' || s.state === 'enemy-mode')) await god(); if (s.state === 'menu' && !s.interlude) { await heal(); return s; } if (s.interlude || s.state === 'text' || (s.state === 'act' && TALK_PHASES.includes(s.gimmick?.phase))) { await press('KeyC', 320); continue; } if (s.state === 'enemy-mode' && s.gimmick && s.gimmick.progress !== undefined && !s.gimmick.broken) { if (cageShots === 0 && !s.gimmick.locked) { cageShots++; await cap('06a_cage_incoming'); } if (cageShots === 1 && s.gimmick.locked && !s.gimmick.mashOpen) { cageShots++; await cap('06a2_cage_prompt'); promptSeen = true; }
...
try {
  ...
  const seenObj = Object.fromEntries(Object.entries(seen).map(([k, v]) => [k, [...v]]));
  check((seenObj.obangsun_beam || []).includes('obeam') && (seenObj.obangsun_beam || []).includes('obangsun_face'), '새 패턴 오방순 빔: 얼굴 + 빔(obeam) ' + JSON.stringify(seenObj.obangsun_beam));
  check(['tank', 'shell', 'mark', 'hline'].every(x => (seenObj.naram_tank || []).includes(x)), '새 패턴 나람 탱크: 탱크·포탄·착지 고리·예고선 ' + JSON.stringify(seenObj.naram_tank));
  check(['warship', 'beam'].every(x => (seenObj.youngcle_ship || []).includes(x)), '새 패턴 영클 함선: 유도 함선 + 고정 십자 레이저 ' + JSON.stringify(seenObj.youngcle_ship));

...
await runScenario({ name: 'battle_bgm', launchOptions: { args: ['--autoplay-policy=no-user-gesture-required'] } }, async ({ page, check, until, open, press, fixture }) => {

await open({ qa: 'teal6' });
await until(() => !!(window.game && game.entities && game.player), 15000); await press('KeyX'); await page.waitForTimeout(300);
await fixture('instrument-audio-and-start-encounter', 'Wrap audio calls to sample element timing/volume, then start the first enemy encounter directly. Headless metadata is not listening evidence.', () => {

...
await goMap('room', 'door');
t = await firstLine(70, 132); check('computer after cord_found', t.includes('코드는 챙겼다'), t);
...
check('autosave written', !!saved && saved.story?.stage === 'cord_found' && saved.map === 'corridor', JSON.stringify(saved && { stage: saved.story?.stage, map: saved.map }));
...
check('continue restores stage/map/flags', s.state === 'field' && s.map === 'corridor' && s.stage === 'cord_found' && s.flags.pc_checked === true && !s.running, JSON.stringify({ state: s.state, map: s.map, stage: s.stage, running: s.running }));
t = await firstLine(300, 224); check('gameplay works after continue (frame line)', t.includes('사진'), t);

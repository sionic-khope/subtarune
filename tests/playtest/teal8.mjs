// 청록숲6 정글 검증: ?qa=teal8 → 나무 많은 구불구불 맵(64×26), 캠프 3(칼날부리·늑대·두꺼비) → 캠프마다 가까이 가서 조우 → 자동 승리 → 40/50/60원·영구 제거·맵 브금 복귀 → 오른쪽 문 → teal9.
...
for (const f of fs.readdirSync(S)) if (f.startsWith('teal8_')) fs.unlinkSync(`${S}/${f}`);
...
const st = () => page.evaluate(() => ({ map: game.mapId, running: game.dialogue.running, box: game.textbox.state, battle: !!game.battle, money: game.money, p: [Math.round(game.player.x), Math.round(game.player.y)], bgm: game.sound.bgmName,
  ...
  trees: game.entities.filter((e) => e.id?.startsWith('jt') && !e.dead).length, flags: Object.keys(game.flags).filter((k) => k.startsWith('teal8_')) }));
...
await page.goto('http://localhost:8000/index.html?qa=teal8'); await until(() => !!(window.game && game.entities && game.player), 15000); await page.keyboard.press('KeyX'); await page.waitForTimeout(400);
let s = await st();
const meta = await page.evaluate(async () => (await import('/src/data/maps.js')).MAPS.teal8.meta);
check('qa=teal8: jungle map with 3 camp monsters (돌거북·바위게·대포미니언) and lots of trees', s.map === 'teal8' && s.enemies.length === 3 && ['krug', 'scuttle', 'cannon'].every((k) => s.enemies.some((e) => e.sprite === k)) && s.trees >= 40 && s.enemies.every((e) => e.img), JSON.stringify({ enemies: s.enemies, trees: s.trees }));
await page.screenshot({ path: `${S}/teal8_01_start.png` });
const rows = await page.evaluate(async () => (await import('/src/data/maps.js')).MAPS.teal8.rows);
check('map is long both ways (64×26), the road bends (left middle → up → right → down → right → up → exit) and camps are far apart', rows.length === 26 && rows[0].length === 64 && /[tuwn]/.test(rows[13][3]) && /[tuwn]/.test(rows[6][12]) && /[tuwn]/.test(rows[6][20]) && /[tuwn]/.test(rows[16][30]) && /[tuwn]/.test(rows[8][50]) && Math.hypot(...[0, 1].map((k) => s.enemies.find((e) => e.id === 'krug')[k ? 'y' : 'x'] - s.enemies.find((e) => e.id === 'cannon')[k ? 'y' : 'x'])) > 1200, JSON.stringify({ rows: [rows.length, rows[0].length], enemies: s.enemies.map((e) => [e.id, e.x, e.y]) }));
// 캠프마다: 몹 옆에 서서 조우 → 전투 자동 승리(적 HP 1) → 돈·제거·브금
const camps = [['krug', 70], ['scuttle', 80], ['cannon', 100]]; let money = s.money;
for (const [id, gain] of camps) {
  ...
  if (id === 'scuttle') { await page.waitForTimeout(500); await page.screenshot({ path: `${S}/teal8_02_battle_${id}.png` }); }
  ...
  check(`${id}: money +${gain}, removed and flagged, map BGM back`, !!after && after.money === money + gain && !after.enemies.some((x) => x.id === id) && after.flags.includes(`teal8_${id}_defeated`) && after.bgm === 'hopes', JSON.stringify(after && { money: after.money, want: money + gain, enemies: after.enemies.map((x) => x.id), flags: after.flags, bgm: after.bgm }));
...
const wantW = ['억빠맨|* 형 여기도 와드 있어요', '경섭|* 이번엔 내가 박는다', '억빠맨|* 형 정글 아니라니까요', '경섭|* 정글은 아닌데 시야는 필요하잖아', '|* 시야가 확보되었다!', '경섭|* 돌거북 바위게 대포미니언?', '억빠맨|* 대포미니언이 왜 정글에 있어요', '경섭|* 길 잃었나 보지. 조심해라 저건 아프다'];
const inOrder = (want, got) => { let i = 0; for (const g of got) if (g === want[i]) i++; return i === want.length; };
check('ward (new lines, 경섭 plants it): lines in order (no gold talk), camera toured far away from the player (≥ 400px), flag set', inOrder(wantW, lines) && camFar >= 400 && !lines.some((l) => /골드|60원/.test(l)) && (await st()).flags.includes('teal8_ward_done'), JSON.stringify({ lines, camFar }));
await page.screenshot({ path: `${S}/teal8_04_ward.png` });
...
check('mana spring (new lines, 억빠맨 drinks first): "마나샘 또 있네" → 빠맨 drinks → 경섭 objects → "형이 먼저 마셨잖아요" → 경섭·요플래 drink → party fully healed (100/120/90) → "다 마셔서 미안"', blue.cols === 3 && ['마나샘 또 있네', '이번엔 저 먼저요', '억빠맨이 마나샘 물을 벌컥벌컥', '시원하다', '마시는 거 아니라며', '먼저 마셨잖아요', '경섭도 옆에서 한 모금', '두 모금 마셨다', '회복되었다', '다 마셔서 미안'].every((k) => lines.some((l) => l.includes(k))) && hp1.join() === '100,120,90', JSON.stringify({ lines, hp1 }));
...
check('mana spring again: short line + heal only (rest point)', lines.some((l) => l.includes('졸졸 흐른다')) && !lines.some((l) => l.includes('또 있네')) && hp2 === 100, JSON.stringify({ lines, hp2 }));
await page.screenshot({ path: `${S}/teal8_03_cleared.png` });
// 출구
const L = await page.evaluate(async () => (await import('/src/data/maps.js')).MAPS.teal8.spawns.landing);
...
const t1 = Date.now(); let mapNow = 'teal8'; while (Date.now() - t1 < 8000) { mapNow = await page.evaluate(() => game.mapId); if (mapNow === 'teal9') break; await page.waitForTimeout(100); }
await page.keyboard.up('ArrowRight');
check('right door → teal9', mapNow === 'teal9', JSON.stringify({ mapNow, preDoor: { running: preDoor.running, box: preDoor.box, p: preDoor.p } }));

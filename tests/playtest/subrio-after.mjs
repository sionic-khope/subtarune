...
// 도트마리오가 비데 머리 위로 떨어짐 → 비데를 들고 왼쪽 문으로 질주 → 연결로(youngcle8) 가로지름 → 무대(youngcle7) 위 통로의 철창 폭파 → 비데 방 복귀·조작.
...
const info = () => page.evaluate(() => {
  ...
  return { map: game.mapId, running: game.dialogue.running, tb: game.textbox.state, bgm: game.sound.bgmName ?? null, zoom: Math.round(game.zoom.s * 100) / 100,
    ...
    bidet: ent('warm_bidet'), mario: ent('mini_mario'), grate: ent('youngcle7_grate'), booms: (game.booms || []).length,
    after: game.flags.subrio_after_done === true, blown: game.flags.youngcle7_grate_blown === true, fade: Math.round(game.fade.alpha * 100) / 100 };
});
...
try {
  ...
  const walkIn = await runUntil(s => !!s.bidet && s.bidet[2] && s.bidet[0] < 900, 'bidet_walk', 30000, 0); check(!!walkIn && walkIn.bidet[0] > walkIn.player[0], '비데가 오른쪽 아래(스크린 아래)에서 일행 오른쪽까지 걸어온다 ' + JSON.stringify(walkIn?.bidet));
  ...
  const passage = await runUntil(s => s.map === 'youngcle7' && !!s.mario && s.mario[1] < 340, 'passage', 20000, 350); check(!!passage && !!passage.grate, '무대 위 통로 철창 아래까지 달려간다(철창은 아직 있음) ' + JSON.stringify(passage && [passage.mario, passage.grate]));
  const blast = await runUntil(s => s.map === 'youngcle7' && s.booms > 0, 'blast', 12000, 250); check(!!blast, '철창 폭파(폭발 이펙트) ' + JSON.stringify(blast && [blast.booms, blast.grate]));
  const blown = await runUntil(s => s.blown && !s.grate, 'blown', 8000, 0); check(!!blown, '철창이 날아가고 youngcle7_grate_blown ' + JSON.stringify(blown && [blown.blown, blown.grate]));

...
const walls = [908, 1132, 1356, 1580, 1804];
...
// ── 3) C → 점프 ──
await page.keyboard.press('KeyC'); let maxJy = 0, swLift = 0, jumpShot = false;
{ const t0 = Date.now(); while (Date.now() - t0 < 2500) { s = await st(); maxJy = Math.max(maxJy, s.raft.jumpY); if (s.sw) swLift = Math.max(swLift, s.sw.lift); if (s.raft.jumpY > 40 && !jumpShot) { jumpShot = true; await page.screenshot({ path: `${S}/void8_04_jump.png` }); } if (!s.raft.jumping && s.raft.x > walls[0] + 30) break; await page.waitForTimeout(30); } }
...
{ const t0 = Date.now(); while (Date.now() - t0 < 4000) { s = await st(); if (s.raft.blocked) break; await page.waitForTimeout(40); } }
...
for (const wx of walls.slice(2)) {
  { const t0 = Date.now(); while (Date.now() - t0 < 4000) { s = await st(); if (s.raft.x >= wx - 112 || s.raft.blocked) break; await page.waitForTimeout(15); } }
...
// ── 6) 도착 ──
{ const t0 = Date.now(); while (Date.now() - t0 < 4000) { s = await st(); if (!s.ride) break; await page.waitForTimeout(60); } }
await page.waitForTimeout(300); s = await st();
check('arrive: raft at end, player on the right shore, swimmer gone, follower visible ahead facing player', !s.ride && s.raft.x === 1952 && !s.sw && s.f?.vis && s.f.x > s.p[0] && s.f.facing === 'left' && s.pf === 'right', JSON.stringify({ raft: s.raft, p: s.p, pf: s.pf, f: s.f, sw: s.sw }));
...
check('reload (autosave at arrival): raft stays at the end, follower present, no cutscene', s.map === 'void8' && s.raft?.x === 1952 && s.f?.vis && !s.running && !s.sw, JSON.stringify({ raft: s.raft, f: s.f, running: s.running }));
await browser.close();

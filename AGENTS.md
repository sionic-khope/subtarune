...
- 생성 스프라이트 요청은 사용 가능한 `pixel-character-sprites` → `imagegen`/`generate2dsprite` 절차를 사용한다. `.claude/skills/art`의 페인터 절차가 사용자 요청의 이미지 생성을 대신하지 않는다. 기존 런타임 규격·정체성은 `assets/source/`와 `docs/handoffs/`를 확인한다.

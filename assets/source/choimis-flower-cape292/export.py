...
def main() -> None:
    ...
    for index, info in enumerate(metadata.frames):
        ...
        old = old_sheet.crop((index % 2 * 128, index // 2 * 128,
                              index % 2 * 128 + 128, index // 2 * 128 + 128))
        old_box = old.getbbox()
        assert old_box
        base = (index % 2 * 320, index // 2 * 184)
        comparison.alpha_composite(old, (base[0] + 16, base[1] + 56))
        ...
        records.append({
            ...
            "feetMedianX": float(np.median(final_x[feet_mask])), "feetBottomYExclusive": box[3],
    ...
    (ROOT / "qc-meta.json").write_text(json.dumps({
        ...
        "headComparisonFrame0": {
            "method": "Approximate visual landmarks: hair top to chin outline; +/-2px uncertainty",
            "oldHairTop": 17, "oldChinBottomExclusive": 69, "oldHeadHeightApprox": 52,
            "newHairTop": 29, "newChinBottomExclusive": 88, "newHeadHeightApprox": 59,
            "oldHeadBodyRatioApprox": 52 / 103, "newHeadBodyRatioApprox": 59 / 123,
            "oldBelowHeadHeightApprox": 51, "newBelowHeadHeightApprox": 64,
        },

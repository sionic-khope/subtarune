...
PREFIX = sys.argv[1] if len(sys.argv) > 1 else 'battle-'   # 'battle-cane-' = 지팡이판(BUILD227)
import numpy as np
...
def pack(raw, row, key):
    ...
    contract[key] = frames
pack(ROOT / f'{PREFIX}idle-raw.png', 0, 'idle')
pack(ROOT / f'{PREFIX}attack-raw.png', 1, 'attack')
sheet.save(OUT / 'janitor.png')
# 달리기: 색키만, 셀 768×512 그대로. pivot = 셀 안 발 밑변 중앙
run = binarize(keyed(ROOT / f'{PREFIX}run-raw.png')); run.save(OUT / 'janitor-run.png')
run_frames = []
...
down = keyed(ROOT / 'battle-down-raw.png'); bb = down.getbbox(); fig = down.crop(bb)   # 쓰러짐은 공통(지팡이 없음)

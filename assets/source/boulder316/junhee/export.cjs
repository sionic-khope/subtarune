...
(async () => {
  ...
  const scale = 85 / (reference[3] - reference[1]);
  const cell = 128, feet = 120, handPlane = 112, frames = [], qc = [];
  for (let i = 0; i < 4; i++) {
    ...
    const x = handPlane - trimmedWidth, y = feet - trimmedHeight;
  ...
  fs.writeFileSync(path.join(__dirname, 'junhee-boulder-push316.png'), sheet);
  fs.writeFileSync(path.join(__dirname, '../../../sprites/junhee-boulder-push316.png'), sheet);
  await sharp(sheet).flatten({ background: '#1b1622' }).resize(768, 768, { kernel: 'nearest' }).png().toFile(path.join(__dirname, 'contact-preview-3x.png'));
  fs.writeFileSync(path.join(__dirname, 'final-qc.json'), JSON.stringify({ scale, pivot: [64, 120], cell: [128, 128], handPlane, frames: qc }, null, 2) + '\n');

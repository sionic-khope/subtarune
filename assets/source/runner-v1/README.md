...
- 7판(BUILD241, 사용자 “검이 너무 길어지고 고개를 숙이고 앞으로 달린다는 느낌이 덜 듦, 무게중심도 살짝 앞으로, 눈이 너무 초롱초롱” + 크리스 달리기 장면[검 손잡이가 몸과 겹치고 무게중심 앞, 검이 기형적으로 길지 않음]): `runner-{run,prep,slash,airslash}7.prompt.txt`, `runner-jump8.prompt.txt` — 몸을 30° 앞으로 기울이고 턱을 당겨 고개를 숙임(시선은 몇 걸음 앞 땅), 눈은 안경 뒤 작은 점(큰 눈·하이라이트 금지, 그림자도 금지), 검은 몸통 길이의 보통 장검으로 두 주먹이 엉덩이에 붙어 손잡이가 몸과 겹치고 칼날은 뒤로 살짝 아래. 참조는 4판과 같은 `runner-ref2.png`. 기준점은 `runner-contract.json`(7판: [흰 셔츠 가운데 x, 발 밑변], scale 0.1435 = 46px / 달리기 평균 키 320.5; 6판 계약은 `runner-contract-v6.json`).
- 8판(사용자 “검 각 살짝만 위로 올리고 오른손의 손바닥면, 왼손은 손등면이 보이는 그 동작”): `runner-{run,prep,slash,airslash}8.prompt.txt`, `runner-jump9.prompt.txt` — 두 손 잡기 규칙에 “가까운 오른손은 손바닥, 뒤의 왼손은 손등이 보이게”를 넣고 칼끝을 살짝 위로. 결과는 칼날이 35~40° 로 너무 들려 채택하지 않음(raw 보관).
- 9판(BUILD243, 채택): 8판 프롬프트에서 각도를 “거의 수평, 10° 만 위”로 못박은 `runner-{run,prep,slash,airslash}9.prompt.txt`, `runner-jump10.prompt.txt`. 계약 `runner-contract.json`(7판 계약은 `runner-contract-v7.json`).
- 올려베기 시트(BUILD244, 사용자 “위로 올릴 때는 턱도 들면서 자세가 잡혀야, 팔만 움직이지 말고 스프라이트를”): `runner-upslash9.prompt.txt` — 웅크림(칼 아래 뒤) → 무릎 옆으로 낮게 베기 → 몸을 펴고 턱을 들어 위를 보며 검을 머리 위로 → 복귀. `assets/sprites/hyungsub-runner-upslash.png`, `CHARACTER_MOTIONS.hyungsub.runner_upslash`.

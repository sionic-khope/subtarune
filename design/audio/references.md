...
| **물걸음 사운드**(얕은 물 위를 걸을 때 — 옵젝영역) | https://www.youtube.com/watch?v=1jZCrBnRm88 (Deltarune sound effects: walking, 사용자 지정 2026-09-12) — **걸음을 다 쓴다**: 원본 5.8초에서 진짜 걸음 33개를 각각 *다음 걸음 직전까지*(104~236ms, 자연스러운 울림 그대로) 잘라 앞 2ms·뒤 12ms 페이드만 주고 70ms 침묵을 사이에 넣어 한 파일로 이어 붙임. 세트 전체에 같은 배율(피크 0.72)이라 걸음마다의 세기 차이는 그대로 | `sfx/water_step.mp3` (7.2초, 조각표 `src/data/footsteps.js`) | **적용됨** 2026-09-12. 타일 `step:{sfx,slices}` → 걸음마다 조각 하나를 `sfx(name,{from,len})` 로 재생(rate 0.96~1.04, volume 0.6). 조각 좌표는 **인코딩된 mp3 를 다시 디코드해 측정**(인코더 지연으로 어택이 잘리지 않게). 4걸음만 잘라 쓴 판(사용자 "너무 짧게 잘라낸 것 같다")·합성 2종·splash 자른 것은 폐기 |

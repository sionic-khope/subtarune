...
- `bgm/save_the_world_full.mp3`(64.3초, BUILD376): 상승·착지·SAVE THE WORLD·결전 한 곡 — 사용자 지정 LAn-JYzKm5M 42.7초~1:47(끝 0.4초 페이드), 원본 `assets/source/rise359/audio/stw.mp3`. 끝나면 `save_the_world_loop`. 이전 `save_the_world_rise`·`save_the_world_run`·`sunset_hum` 은 쓰지 않음.
...
- `bgm/heart_rise.mp3`(24.3초, BUILD364): 결말 하트 상승·세로 빛의 파장. 사용자 지정 YouTube wHAircQnFx0 전체, 한 번(루프 없음) 뒤 `wind`. 원본 `assets/source/epilogue364/audio/heart.mp3`.
- `bgm/lounge_parade.mp3`(80초, BUILD364): 라운지 결말 퍼레이드. 사용자 지정 YouTube P89rxnT7lKw 전체. 원본 `assets/source/epilogue364/audio/parade.mp3`.
...
사용자가 지정 더빙 목소리를 참고해 새 문장을 만들도록 요청해, 기존 [T3THgeD8bpI](https://www.youtube.com/shorts/T3THgeD8bpI)의 짧은 반응을 참조한 로컬 Qwen3-TTS 합성을 사용했다. 원본 성우가 말하지 않은 합성 대사이며 원본 녹음으로 표시하지 않는다. 파일은 `sfx/choimis_flower_hello|seup|sexy|gap|gonik.mp3`, 문장은 각각 ‘안녕하세요 형들’, ‘스읍 미스’, ‘나 섹시해’, ‘GAP티 입을래?’, ‘난 가재맨 고닉’이다. 원본·합성 파라미터·전사·정규화·파일 검사는 `assets/source/choimis-voice292/README.md`를 기준으로 한다.

기존 표시 대사에 반응 음성을 배치하며 음성 노드는 텍스트 전체 동안 `voice:none`이다. 소리가 먼저 끝나도 블립을 재개하지 않는다. 로딩이 늦으면 재생 준비 후 노드를 시작하고 취소된 장면에서는 늦게 재생하지 않는다. 자동 전사의 ‘습, 미스’와 ‘캡티 입을래?’는 발음 모호성을 남기므로 원문 정확도·참조 목소리 동일성에 대한 사람의 청취 승인으로 간주하지 않는다. 외부 유료 TTS API 호출은 없었으며 로컬 계산 비용은 측정하지 않았다. 출처 기록과 사용자의 제작 요청은 원 권리자의 이용허락 확인을 뜻하지 않는다.

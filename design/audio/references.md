...
| **물걸음 사운드**(얕은 물 위를 걸을 때 — 옵젝영역) | https://www.youtube.com/watch?v=1jZCrBnRm88 (Deltarune sound effects: walking, 사용자 지정 2026-09-12 "몇 걸음 섞여 있음") — 5.8초에 걸음 29개. 앞 여유 ≥0.24s 로 꼬리가 안 겹치는 **걸음 4개**(1.13·2.88·4.41·5.28초)를 각 200ms 로 잘라 앞 2ms 페이드인·뒤 45ms 페이드아웃·피크 -6dB 통일 | `sfx/water_step.mp3`, `water_step2`, `water_step3`, `water_step4` | **적용됨** 2026-09-12. 타일이 `step:[…]` 로 선언하면 걸음마다 하나를 골라 rate 0.94~1.06·volume 0.6 으로 재생(`Player.footstep`). 앞선 합성 시도 2종(짧은 첨벙 "빈도 많고 쫀득", 물방울 울림 "너무 별로")과 splash 잘라 쓴 3차는 폐기 |

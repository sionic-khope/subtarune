...
- `rhythm/tvtime.json`(BUILD218 리듬 특별 패턴 차트): `bgm/youngcle_tvform_battle.mp3` 에서 `tools/rhythm/chart.py --player melody --no-video`(300~4000Hz 스펙트럼 플럭스 온셋, 최소 간격 0.16s, 2초 창 7개 상한)로 뽑은 플레이어 노트 398개(2.3/s, 홀드 없음) + 사이드 drums 538/vocal 402. 사용자 “멜로디에 맞게 떨어지는 거야” — 박자 격자 양자화 없음. 노트 창은 지금 브금 시각+2초부터 15초.

...
| **물걸음 사운드**(얕은 물 위를 걸을 때 — 옵젝영역) | https://www.youtube.com/watch?v=1jZCrBnRm88 (Deltarune sound effects: walking, 사용자 지정 2026-09-12) — **걸음을 다 쓴다**: 원본 5.8초의 걸음 33개를 각각 *어택 앞까지 되짚어* 시작점을 잡고(그 걸음 피크의 3%) *다음 걸음 직전까지*(98~230ms) 잘랐다. 앞 1.5ms·뒤 10ms 페이드(클릭만 방지), **세트 전체에 같은 배율 한 번**(걸음마다의 세기 차이 0.11~0.85 는 원본 그대로 — 낱개 정규화하면 영상과 다르게 들린다) | `sfx/water_step01.mp3` ~ `water_step33.mp3` (목록 `src/data/footsteps.js`) | **적용됨** 2026-09-12. 타일 `step` 배열에서 걸음마다 파일 하나를 **그대로**(음높이 안 바꿈, volume 0.75) 재생. **한 파일 안에서 구간만 재생하는 방식은 금지** — 브라우저가 mp3 `currentTime` 을 무시해 걸음마다 파일 맨 앞의 같은 소리만 났다("걸음소리 더 이상함"). 합성 2종·splash 자른 것·4걸음만 쓴 것도 폐기 |

...
| `sfx/guitar_lead_{a2,a3,a4}.mp3`, `sfx/guitar_dead.mp3` | 합성 `tools/audio/guitar.py`(BUILD182: 리드 = Karplus-Strong 두 현 + 게인 18 앰프, 기준음 A2/A3/A4; 데드 노트 = 로우패스 노이즈 + 70Hz 툭) — 사용자 “기타가 노래랑 안 조화롭다·너무 시끄럽다”, “음이 노래 음이랑 똑같이 가게” | 0.55s / 0.1s | GREAT 를 차트의 멜로디 pitch 로 이조해 재생(`Sound.sfx pitch:true`), MISS 툭. `guitar_pc_e/a`·`guitar_c4/g4/a4`·`guitar_mute` 는 더 안 씀(파일 보존) |
| `assets/video/noamtori.mp4` | 원본 유튜브 -IvdHDCBsps 를 18.2초(‘만원 주면 넘어줘요 앰토리’)부터 잘라 libx264 crf24 + aac 128k 재인코딩(BUILD182, 사용자 “앰토리는 만원주면~ 부터”). 전체본은 git 82e80a8 | 69.6s | 리듬 첫 곡 |

...
| **물걸음 사운드**(얕은 물 위를 걸을 때 — 옵젝영역) | https://www.youtube.com/watch?v=1jZCrBnRm88 (Deltarune sound effects: walking, 사용자 지정 2026-09-12) — 원본은 걸음이 0.07~0.27초 간격으로 이어져 **어느 걸음도 울림이 온전하지 않다**(다음 걸음에 가려짐). 사용자 2026-09-12 "그냥 너가 이거처럼 새로 만들면 안 되나" → 잘라 붙이지 않고 걸음 33개를 각각 **영상 소리를 본떠 새로 렌더링**: 건조음 = 그 걸음의 첫 찰싹(어택 6ms 앞 ~ +100ms, 끝 40ms 페이드 — 영상 소리 그대로) ⊛ 방 임펄스 응답. IR 은 영상에서 잰 것으로 설계 — 감쇠 **-43dB/s(RT60 1.4초, 원본 268ms 간격 안의 잔향 기울기)**, 음색 = 원본 잔향 스펙트럼 ÷ 건조음 스펙트럼(1/3옥타브), 50ms 차오름(원본 잔향이 +60ms 에서 가장 큼), 세기는 걸음마다 +80~110ms 울림이 **원본의 그 구간 레벨(-40.8dB, 간격 긴 걸음 21개 중앙값)** 이 되게(원본은 큰 걸음이든 작은 걸음이든 울림 크기가 같다). 검증: 간격 긴 걸음 21개에서 원본과 포락선 오차 평균 2.2dB(+20~200ms). -72dBFS 까지 + 40ms 페이드. 생성기 `tools/audio/water_steps.py --src <원본 wav> --preview <폴더>`(걷기 0.67s·달리기 0.37s 시연 wav + 다음 걸음 직전 레벨: 걷기 ≈ -60dBFS, 달리기 ≈ -48 — 바람 브금 1~3kHz 대역 -55). 세기는 **세트 전체 한 배율**(어택 크기 차이는 원본 그대로) | `sfx/water_step01.mp3` ~ `water_step33.mp3` (0.95~1.04초, 목록 `src/data/footsteps.js`) | **적용됨** 2026-09-12. 걸음마다 파일 하나를 **그대로**(음높이 안 바꿈, volume 0.75) 재생. 폐기된 시도: 합성 2종 · splash 자른 것 · 4걸음만 · 한 파일 구간 재생(브라우저가 `currentTime` 무시) · 잔향 없이 다음 걸음 직전에서 끊은 것("끊기는 느낌") · 마지막 걸음 잔향에서 잰 -109dB/s 로 0.4초만 이어 붙인 것("아직 끊기는 기분" — 원본은 걸음 사이 잔향 바닥이 -43dB 로 두 배 넘게 느리게 꺼진다) · 잔향 알갱이를 -43dB/s 로 1.2초 이어 붙인 것(사용자 "그냥 새로 만들면 안 되나" → 렌더링 방식으로 교체) |

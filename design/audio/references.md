...
| **물걸음 사운드**(얕은 물 위를 걸을 때 — 옵젝영역) | https://www.youtube.com/watch?v=1jZCrBnRm88 (Deltarune sound effects: walking, 사용자 지정 2026-09-12) — 원본은 걸음이 0.10~0.23초 간격으로 이어져 **모든 걸음의 잔향이 다음 걸음에 가려져 있다**(온전한 건 마지막 하나뿐). 그래서 걸음 33개를 각각 [어택 앞까지 되짚은 시작 ~ 다음 걸음 직전까지의 진짜 소리] + [같은 방의 잔향 꼬리] 로 만든다: 꼬리 재료는 **간격 160ms 이상인 걸음들의 잔향 구간**(같은 녹음·같은 방)을 평탄화해 80ms 알갱이로 이어 붙인 것, 감쇠는 원본 268ms 간격 안에서 잰 **-43dB/s(RT60 1.4초)**, -86dB(게임 출력 약 -72dBFS)까지 + 40ms 페이드. 이음매 16ms 등전력 크로스페이드, 어택 14ms 앞 6ms 페이드인. 생성기 `tools/audio/water_steps.py --src <원본 wav> --preview <폴더>`(걷기 0.67s·달리기 0.37s 간격 시연 wav 와 다음 걸음 직전 레벨을 출력: 걷기 ≈ -60dBFS, 달리기 ≈ -46 — 바람 브금 1~3kHz 대역 -55). 세기는 **세트 전체 한 배율**(걸음마다의 차이는 원본 그대로) | `sfx/water_step01.mp3` ~ `water_step33.mp3` (1.05~1.34초, 목록 `src/data/footsteps.js`) | **적용됨** 2026-09-12. 걸음마다 파일 하나를 **그대로**(음높이 안 바꿈, volume 0.75) 재생. 폐기된 시도: 합성 2종 · splash 자른 것 · 4걸음만 · 한 파일 구간 재생(브라우저가 `currentTime` 무시) · 잔향 없이 다음 걸음 직전에서 끊은 것("끊기는 느낌") · 마지막 걸음 잔향에서 잰 -109dB/s 로 0.4초만 이어 붙인 것("아직 끊기는 기분" — 원본은 걸음 사이 잔향 바닥이 -43dB 로 두 배 넘게 느리게 꺼진다) |

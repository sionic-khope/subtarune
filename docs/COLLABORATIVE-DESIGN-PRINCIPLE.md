...
> 섭타룬 적용 범위(2026-09-14): 아래 내용은 도입 당시의 범용 스튜디오 협업 예시다. 현재 실행 권한·기획/구현 구분·메인 반영은 [프로젝트 공통 판단 기준](development/quality-contract.md)이 원본이다. 사용자가 이미 맡긴 작업에 아래의 파일별 쓰기 승인·검사별 승인·무조건 질문 절차를 재적용하지 않는다. 사용자가 별도로 인터뷰/초안 승인을 요청할 때 그 범위의 예시로만 읽는다.

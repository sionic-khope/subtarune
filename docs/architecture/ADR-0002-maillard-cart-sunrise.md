...
.110에서 선로 길이를3072px로 늘려128px/s·24초 주행으로 수정했다. `Raft`의 `rideTime`은 탑승 시0으로 초기화되고 실제 이동 중에만 누적된다. 데이터 `passengerLookAfter:8/passengerLookFacing:'up'`로 승객 시선을 바꾸되 `dirFacing`과 이동 경로는 보존하며 하차 시 진행 방향을 복원한다. 지정되지 않은 기존 뗏목의 동작은 유지된다.

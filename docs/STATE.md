...
- 캐릭터 배율 `CHAR_SCALE=1.43`(world.js, 2026-09-09 +10%). 기본 이동 = 달리기, X/Shift = 천천히. C 확인, X 취소, Esc 타이틀, V 메뉴, **Tab 비상탈출**(끼었을 때: 창에서 탈출 → 이 맵 입구 스폰으로 재진입. **스토리 플래그는 유지**, 되돌려도 되는 맵 장치(뗏목)만 입구 쪽 끝으로 되돌림, 탈것·헤엄·피격 정리 — `main.js doEscape` 주석이 설계 원칙, 2026-09-10), T(타이틀) 테스트룸, F1 디버그(오디오 상태 포함).

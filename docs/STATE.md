...
- **`void3` 뗏목 퍼즐**(36×24): 입구 A → 뗏목 → 교차로 B(위·오른쪽·아래 뗏목 셋) → 아래 D·오른쪽 G 는 막다른길(표지판 "막다른 길이다 / 내려온 땟목을 다시 타면 돌아간다", "오 이걸 찾았노 ㅊㅋㅊㅋ"(사용자 지정)) → **위 C → 오른쪽 F → 아래 E → 거대한 검은 문**(`void_door` 임시). 표지판 대사는 내가 지음(껄껄 톤).

...
- **2026-09-10 피드백 묶음(포스트모텀 `docs/postmortems/2026-09-10-rockfall-door-feedback.md`)**:
  ...
  - **요청 없는 효과음은 넣지 않는다**(낙석 착지·피격 무음). 흰색/밝은 섬광·가운데 선 같은 "빛 연출" 도 요청 없으면 부드러운 한 겹만.
...
- **낙석 맵 3개** `void5`(3개) → 오른쪽 끝에서 아래로 → `void6`(6개) → 아래로 → `void7`(9개) → 오른쪽 끝 뚫림 → `void8`(빈 착지, 다음 브리핑 대기). 살짝 어두움(`dim:0.3`) + 보라 불 배경. 입구에서 **8타일 지나서** 첫 레인, 레인 간격 **5타일**, 마지막 레인 뒤 6타일 (void5 30열·void6 48열·void7 60열). 낙석은 캐릭터 폭(28px, 손그림 도트), 2초 주기 왼→오 리듬. 떨어지기 전 0.8초 동안 **부드러운 연보라 스포트라이트**(가운데 선·흰 섬광 없음)가 그 자리를 비춘다 → 화면 위에서 **길의 모든 줄을 쓸고 내려와** 맨 아랫줄에 착지 → 0.45초 놓였다 사라짐. **소리 없음.** 맞으면(바위가 실제로 지나갈 때만) 붉은 섬광·흔들림·**왼쪽으로 슬라이드**(동료도 같이, 체력 없음), 0.9초 무적.

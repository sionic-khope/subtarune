...
- **`void10` 미로**(46×34, `tools/maps/void10.py`, 사용자 브리핑 2026-09-10 "물 없는 보라 땅 미로, 대각선 아래가 목표, 걷기만, 표지판 중간중간"): 셀 15×11(통로 2타일·벽 1타일) 되돌아가기 미로, 정답 41셀. 왼쪽 위 시작 → 오른쪽 아래 **포탈**(`portal.png`, 캐릭터 위에 덮여 그려짐 `sortY`) → `void11`. **도착 컷신**(`enter`, `src/data/cutscenes/void10_maze.js`): 카메라가 출구로 → 브금 꺼짐 → 쥰희 "형 빨리 오샘" → 포탈로 들어가 사라짐(whoosh) → 경섭 "어 그래 어휴.." → 따라 들어감 → 브금 복귀 → 카메라가 주인공에게(`void10_intro`, 두 NPC 는 `unless`). **표지판 5개**(정답 길 옆 막다른 셀, 진행 순서·서로 4셀 이상 떨어짐): 1 "여기는 외딴섬 여기는 외딴섬" / 2 "아 아직 여기 계시는 군요" / 3 "0/0/0 이에요 저는 당신은?" "아 저는 0/32/1 이요." → 나레이션 "뭐라는거지" / 4 "치지직 ... ×3"(흔들림 태그) / 5 "나갈 수 없어 너만큼은 나갈 수 없어."(출구에 가장 가까움). 시드·표지판 셀은 JSON `meta`.
- **`void11` 거대 나무**(44×16, `tools/maps/void11.py`, 브리핑 "가로로 조금 긴 맵, 가운데 거대한 나무(고퀄 도트), 쥰희·경섭 컷신 → 경섭 합류"): `tree_big.png`(240×264, `tools/art/void10_set.py` — 잎 뭉치 원 16개·좌상단 광원 4단·겹침 스캘럽·뿌리) 가운데, 밑동만 막힘. 브금 **Lancer**(`bgm/lancer.mp3`, `bgmFlag: void11_intro` → 첫 도착은 조용, 컷신이 쥰희 첫 대사에서 켬). **도착 컷신**(`enter.early` — 검은 화면 아래에서 카메라부터, `src/data/cutscenes/void11_tree.js`, 대사 브리핑 그대로 31줄): 쥰희·경섭 다툼 → 식은땀(`emote sweat`) → 웃음(`motion laugh`) → "코앞이야 우리들의 세계" → 빠르게 다가가 "{c=yellow}그것{/c}" → 물러남 → 으하하 → 통나무(`logs.png`) 뺏고 오른쪽으로 달려 사라짐 → 경섭 "ㅅㅂ인생" → 뒤돌아 느낌표(`emote !`) → 카메라가 형섭·억빠맨에게, 둘이 경섭 앞으로 걸어옴 → 대화 → "뭔가 살짝 수상하지만 경섭이 동료가 되었다." → `{join:'gyeongsub'}`(파티 2명, `void11_done`) → 오른쪽 출구 `void12`(빈 착지, 브리핑 대기).
...
- **뗏목** `src/world/world.js Raft` (`type:'raft'`): `{ type:'raft', id, image:'assets/props/raft.png', x,y, route:[[x,y],…], speed:114 }`. 옆에서 C → route 를 따라 일직선 이동(타는 동안 `game.ride` 가 서서 입력·트리거 정지), 도착하면 진행 방향으로 밀어 내림, 반대편에서 타면 되돌아옴. 위치는 `flags.raft_<id>`(route 인덱스)로 유지 → 맵을 나갔다 와도 그 자리. 속도 기본 171px/s(2026-09-09 +50%). 소리: 탈 때·1.1s 마다·내릴 때 `splash`(합성 첨벙, 2026-09-10 더 물소리답게). 물 타일 `o/O`(완전 단색 파랑, 막힘). 뗏목 도트는 최소(외곽선+한 색+선 3개) — 델타룬식, 요청 없는 소품은 디테일 넣지 않는다. 새 맵에 그대로 복사해 route 만 바꾸면 됨. 퍼즐 예시 `void3`(교차로 + 막다른길 2개). 검증 `tests/playtest/raft.mjs`, `void3.mjs`.
  ...
  - 맵 `enter.early: true`: 도착 스크립트를 검은 화면이 걷히기 **전**에 시작 — 첫 노드 `{camera:[..], duration:0.01}` 로 옮겨 두면 플레이어가 한 프레임도 안 보인다(void11). 맵 `bgmFlag:'플래그'`: 그 플래그가 켜진 뒤에만 맵 브금(그 전엔 조용히) — 컷신이 대사 중간에 브금을 켜는 맵.

...
- **청록숲**(`tools/maps/teal.py`, 사용자 브리핑 2026-09-10 "청록색 타일·검은 배경·애매하게 보이는 수풀 배경"): 타일 `t/u`(청록 땅 체커) `g`(잔풀 땅) `v`(절벽면, 막힘) — `tools/art/teal_set.py` → `assets/tiles/ground_teal*.png, grass_teal.png, cliff_teal.png`. 배경 `backdrop:'teal_bush'`(`main.js drawBackdropTeal`: 검은 어둠 속 3겹 덤불 실루엣, 잎 점·줄기, 시차·느린 흔들림).
  - **`teal1`**(64×12): void11 오른쪽 출구 → 오른쪽으로 쭉 가는 길. 브금 **Weird Birds**(`bgm/weird_birds.mp3`, 16초 루프). **꽃가루 풀 3마리**(`Spitter` 엔티티 `world.js`, `spitter.png` 2프레임, 길 위쪽 가장자리 1/4·2/4·3/4 균등, 주인공이 300px 안에 오면 2.6초마다 흰 꽃가루 9알을 주인공 쪽으로 뿜음 — 맞아도 아무 일 없음, 사용자 지시).
  - **`teal2`**(44×26): 가로 길(19~23행) 오른쪽 출구(42열)보다 **3칸 왼쪽(39열)에 쥰희 닮은 나무 동상 벽**(`statue_junhee.png` 44×60, 세로 5개, 받침만 막힘) → 오른쪽 막힘(`teal_east` 빈 착지). 그 바로 왼쪽 35~37열에서 **위로 가는 길**(→ `teal3` 빈 착지, 다음 브리핑). 길 가장자리·위 길 옆에 동상 8개가 깔려 있다(조사: "나무 동상이다. 어디서 본 얼굴 같다."). 브금 **Field of Hopes and Dreams**(`bgm/hopes.mp3`). **가운데 광장**(13~28열·12~23행, 사용자 "가운데 중앙은 좀 더 넓게")에 **이벤트 나무**(`tree_teal.png`, `teal2_tree` — 내가 쓴 연출: 빠맨 "이 나무 뭔가 얼굴 같지 않아요?" → 똑똑 두드림(`sfx/knock.mp3`, 언더테일 snd_knock) → 잠시 뒤 안에서 똑똑 되돌아옴(작은 흔들림) → 빠맨 느낌표 "지금 안에서 두드린 거 맞죠?" / 경섭 "허허 바람 소리겠지" → 빠맨이 형섭 뒤로 숨음 "저 여기 있을게요" / "... 확실히 두 번 두드렸다.", 다시: "저건 다신 안 두드릴래요") + **바나나 2개**(`banana.png`, `teal2_banana1/2`, 대사 그대로: 빠맨 "형섭이형 바나나 드세요" [먹는다 → "포타슘", 바나나 사라짐(`banana1_eaten`) / 안먹는다 → "몸상하세요"]). **벽 동상에 C**(`src/data/cutscenes/teal2_statue.js`, 대사 그대로): 빠맨 "음 이 더러운 동상은 뭐지? 타코 닮았어요" / 경섭 "허허 지나갈 수 가 없네" / 빠맨 "부숴버리면 되는거죠" → 빠맨이 동상 앞으로 달려가 몸통 박치기(`hop`)·쿵·흔들림·튕겨 나옴·식은땀 → "아 존나 아프다 씨발 이거 왜이렇게 단단해" / 나레이션 "... 다른 방법을 찾아봐야겠다." → 카메라가 위 길을 잠깐 비춤(유도) → `statue_hit`. 다시: "아주 단단하다" / "저건 다신 안 건드릴래요".

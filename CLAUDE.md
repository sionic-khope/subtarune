...
2. 맵은 `/map`, 컷신은 `/cutscene`, 적·전투는 `/enemy`; 아이디어는 `/subtarune-design`, 메뉴는 `/subtarune-ui`, 생성 그림의 담당 전달은 `/subtarune-sprite-handoff`, 완료 검수는 `/subtarune-verify`를 적용한다. 실제 구현의 메인 반영은 `docs/STATE.md`의 “메인 반영·전투 확장 방향”을 따른다.

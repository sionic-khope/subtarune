...
# ──────────────────
"""용암 수로(BUILD192): 입구에서 오른쪽으로 걸으면 용암 위 뗏목. 수로는 한 줄(3칸 96px). 구간 ① 오른쪽 → 착지 → 구간 ② 좁은 세로 수로로 위로 → 위 착지 → 구간 ③ 다시 오른쪽(사용자 2026-09-16: “위로 가는 용암 퍼즐 후에 오른쪽으로 가는 용암길도”) → 오른쪽 끝은 열린 통로(다음 맵은 다음 브리핑. 사용자가 거부한 철문 같은 소품을 지어내지 않는다).

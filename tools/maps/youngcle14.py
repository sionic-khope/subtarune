...
# ──────────────────
"""용암 수로(BUILD191): 입구에서 오른쪽으로 걸으면 용암 위 뗏목. 수로는 한 줄(3칸 96px). 구간 ① 오른쪽 → 착지 → 구간 ② 좁은 세로 수로로 위로(사용자: “위로 가는 거 없애버린 거야?” — 다시 넣음) → 위 착지 끝의 잠긴 철문(다음 브리핑).
...
LANE_TOP = 20 * T                                      # 640
# 구간 ② 세로 한 줄 수로 cols 54~56 · rows 3~19(96px 폭), 뗏목은 착지 바닥 바로 위(y600)에서 위로 y104 까지
RAFT_B_X, RAFT_B_Y0, RAFT_B_Y1 = 54 * T + 20, 20 * T - 40, 3 * T + 8   # 1748, 600, 104
...
def beam(id_: str, x: int, high: bool, pulse: dict | None = None, oscillate: dict | None = None) -> dict:
    ...
    e = {'type': 'prop', 'id': id_, 'image': f"assets/props/{'plasma_beam_high' if high else 'plasma_beam_v'}.png", 'x': x, 'y': LANE_TOP, 'w': 40, 'h': 96,
         'ix': x, 'iy': LANE_TOP - (64 if high else 0), 'sortY': LANE_TOP, 'solid': True, 'obstacle': True, 'clear': 80 if high else 40, 'sweep': True, 'anim': {'cols': 3, 'fps': 10}}
...
def main() -> None:
    ...
    # 입구 바닥 rows 20~22 · cols 1~12 / 구간 ① 용암 한 줄 rows 20~22 · cols 12~52 / 착지 바닥 rows 20~22 · cols 52~58
    for row in range(20, 23):
        ...
        for col in range(52, 59):
            cells[row][col] = 'F'
    # 구간 ② 세로 한 줄 용암 cols 54~56 · rows 3~19 / 위 착지 바닥 rows 1~2 · cols 52~58
    ...
    map_data = {
        ...
        'preload': ['assets/tiles/youngcle_iron_blue.png', 'assets/tiles/lava.png', 'assets/backdrops/youngcle_furnace.png',
                    'assets/props/raft.png', 'assets/props/plasma_beam_v.png', 'assets/props/plasma_beam_high.png', 'assets/props/lava_wall.png', 'assets/props/lava_wall_low.png', 'assets/tiles/youngcle_iron_blue_wall.png', 'assets/props/plasma_beam_v_h.png', 'assets/props/plasma_beam_high_h.png', 'assets/props/iron_door.png'],
        'spawns': {
            'left': {'x': 56, 'y': 680, 'facing': 'right'},
            'landing': {'x': 1700, 'y': 680, 'facing': 'right'},
            'top': {'x': 1700, 'y': 56, 'facing': 'right'},
        },
        'meta': {'connected': True, 'route': [[2, 21], [10, 21]], 'raft': [RAFT_X0, RAFT_Y, RAFT_X1], 'raftB': [RAFT_B_X, RAFT_B_Y0, RAFT_B_Y1], 'checkpoints': checkpoints, 'checkpointsB': checkpoints_b},
        'entities': [
            ...
            # 부두 안내판(장애물 구분)
            {'type': 'sign', 'id': 'lava14_hint', 'x': 320, 'y': 632, 'w': 24, 'h': 16, 'solid': True,
            ...
            beam_h('beam_b_high1', 230, True, pulse={'on': 1.4, 'off': 1.2, 'phase': 0.4}),
            # 위 착지 오른쪽 끝: 잠긴 철문(보이는 소품) — 다음 지역은 다음 브리핑
            {'type': 'prop', 'id': 'lava14_end_door', 'image': 'assets/props/iron_door.png', 'x': 1856, 'y': 44, 'w': 32, 'h': 52,
             'ix': 1852, 'iy': 40, 'solid': True, 'script': 'lava_end_door'},
        ],
    }

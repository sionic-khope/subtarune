...
"""옵젝영역0 (사용자 브리핑 2026-09-11): 청록숲9 사원 문을 지나면 나오는 새 지역. 일직선 길 하나 — 바닥은 얕은 물(밟으면 에코 물 발소리 + 물결 고리, tiles.js step),
  주변은 더 울창한 초록숲(보라 나무를 넷에 하나꼴로 섞음), 배경 backdrop obj_forest(초록 덤불 + 보라 먼 층), 브금은 허공처럼 '휘잉' 바람(wind). 이벤트·몹 없음(다음 브리핑).
...
rows0 = build(W, H, R0, R1, ents0, True)
m0 = {'id': 'obj0', 'name': '옵젝영역', 'bgm': 'wind', 'stage': 'void_fallen', 'dim': 0, 'backdrop': 'obj_forest', 'rows': rows0,
      ...
      'meta': {'connected': True, 'road': [R0, R1], 'trees': len([e for e in ents0 if e.get('id', '').startswith('ot')])},
      'entities': ents0}

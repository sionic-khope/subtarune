...
형섭 옆모습 원본은 중립–발 뻗음–발 뻗음–중립 순서라, `src/data/characters.js`의 `walkFrameOrder`가 좌우만 `[0, 1, 3, 2]`로 재생한다(0부터 시작). PNG 열은 원본 그대로이며 다른 방향·캐릭터에는 이 순서를 무조건 적용하지 않는다. 게임용 PNG를 다른 도구에 줄 때는 4열×4행 및 이 재생 설정도 함께 전달한다.

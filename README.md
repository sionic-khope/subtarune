...
### 암흑 추격자·기존 장면 교정 (BUILD319)

검은 길 다음 미로의 추격자는 검은 구형 몸체·보라 오라·밝은 중앙 코어를 가진 새 별도 캐릭터다.192×192 RGBA 한 장·중심 피벗96,96을 월드 이동시키며, 외곽 오라는 길을 밝히거나 충돌하지 않는다. 원본·프롬프트·알파/크기 처리는 [추격자 자산 계약](assets/source/castle319/pursuer/README.md), 벽 통과 추격·접촉 재시도·회복 샘과 마지막 닫힌 문은 [추격 콘티](design/narrative/cutscenes/castle_dark_chase.md)를 따른다.

기존 동료 시트·표시 크기는 유지한다. 로비의 전원 암전 배치와 안전한 보행은 [대문 콘티](design/narrative/cutscenes/castle_gate.md), 거석 초점 발사음·From Now On 전환·10단계 카메라는 [거석 콘티](design/narrative/cutscenes/castle_boulder.md)가 최신이다. 검은 길은 이동80px 간격의 넓은 파동만으로 길을 드러낸다. 아래318의 빈 도착 맵 범위는 이번 추격 미로·안전 방으로 확장됐으며, 검증과 배포 여부는 [STATE](docs/STATE.md)에서 분리한다.

### 중앙 대문 개방·암흑 파동 길 (BUILD318)

양쪽 구체 활성화 뒤 동료들과 중앙 문을 여는 후속 연출이 이어진다. 아래316/311의 “대문은 아직 닫힘”은 당시 진행 기록이며318이 대체한다. 열린 문은 기존320×384 문틀과 동일한 캔버스·피벗을 유지한 별도 생성 소품이다. 원본·프롬프트·알파 보존 내보내기는 [열린 대문 자산 계약](assets/source/castle318/gate/README.md)을 따른다. 길은 완전 검정이며 발걸음 파동이 실제 길 경계에서 끊겨 위→오른쪽→위의 방향을 찾는다. 기존 얼굴·오브제 발소리를 재사용하고 새 캐릭터 디자인은 만들지 않는다.

### 거석 다리·누누섭·쥰희 밀기 (BUILD316)

BUILD317 표시 교정: 원본 PNG·프레임·피벗은 보존하고 누누 장면 배율은1.3, 거석과 박힌 잔해는1.5배(거석 실체 약330px)로 통일한다. 아래0.88/220px는 제작 당시 규격이다. 파크가디언은 라운지와 같은 인형탈 `park_guardian_costume`2.22, 뚜울라는 `ttuulla`1.79를 그대로 사용한다. 최신 구도·밀기 전환은 [DESIGN](DESIGN.md#거석-협동-밀기-교정-build317)을 따른다.

누누섭은 작은 기수와 네 팔의 거대한 설인에 가재맨의 안경·두 눈·넓은 볼·큰 코, 회청/먹색 어둠 팔레트를 결합한다.384px셀2×2·발 피벗192,360·장면 배율0.88이며 밀기 두 포즈·포효·밀려남을 구분한다. [원본과 모션 규격](assets/source/boulder316/nunusub/README.md)을 따른다. 쥰희는 기존 분홍 몸·파란 눈·큰 코·짧은 팔다리를 유지한 별도 양손 밀기4포즈로,128px셀·발64,120·손 접촉면x112·모션 배율0.5다. 기존 걷기/웃음은 바꾸지 않는다. [쥰희 규격](assets/source/boulder316/junhee/README.md).

거석은 실제220px 지름이며 굴러가는 그림과 벽에 박힌 그림의 크기·접점을 같게 연결한다. [거석/벽 자산 계약](assets/source/boulder316/props/README.md)을 따르고 소품 생성기의 중간 출력으로 최종 PNG를 덮어쓰지 않는다. 여섯 비석·협동 밀기·왼쪽 봉인까지의 진행은 [콘티](design/narrative/cutscenes/castle_boulder.md), 검증·배포 상태는 [STATE](docs/STATE.md) 기준이다. 왼쪽 구체 점등 후에도 중앙 대문은 아직 열지 않는다.
